...
static esp_err_t
transport_set_receive_callback(enp_transport_receive_callback_t callback) {
	(void)callback;
	return ESP_OK;
}
...
static esp_err_t reliability_submit(const enp_packet_t *packet,
									void *user_context) {
	(void)user_context;
...
void app_main(void) {
	...
	pass &=
		expect(enp_context_init(&s_context, &s_transport, &config) == ESP_OK,
			   "ENP context initialized with controlled transport");

	pass &= expect(s_transport_init_count == 1U,
				   "controlled transport initialized once");

	pass &= expect(enp_reliability_init(), "reliability initialized");

	pass &=
		expect(enp_reliability_set_submit_callback(reliability_submit, NULL),
			   "reliability submit callback configured");

	pass &=
		expect(enp_reliability_set_result_callback(reliability_result, NULL),
			   "reliability result callback configured");

	pass &= expect(enp_reliability_start(), "reliability started");

	pass &= expect(enp_maintenance_init(&s_context) == ESP_OK,
				   "ENP maintenance task started");

	pass &= expect(make_data(&s_data), "reliable DATA packet constructed");

	enp_reliability_handle_t handle = ENP_RELIABILITY_INVALID_HANDLE;

	const uint32_t send_time = enp_context_time_ms(&s_context);

	pass &= expect(enp_reliability_send(&s_data, send_time, &handle),
				   "reliable DATA transaction submitted");

	pass &= expect(s_reliability_submit_count == 1U,
	...
	pass &= expect(enp_reliability_get_retry_count(handle, &retry_count) &&
					   retry_count == 1U,
				   "reliability transaction recorded exactly one retry");

	pass &= expect(memcmp(&s_data, &s_last_submitted, sizeof(s_data)) == 0,
				   "maintenance-driven retransmission preserved DATA "
				   "transaction identity");

	pass &= expect(make_ack(&s_ack), "correlated ACK packet constructed");

	pass &= expect(
		enp_reliability_process_ack(&s_ack, enp_context_time_ms(&s_context)),
	...
	pass &= expect(enp_maintenance_deinit() == ESP_OK,
	...
	pass &= expect(enp_context_deinit(&s_context) == ESP_OK,
				   "ENP context deinitialized");

	pass &= expect(s_transport_deinit_count == 1U,
