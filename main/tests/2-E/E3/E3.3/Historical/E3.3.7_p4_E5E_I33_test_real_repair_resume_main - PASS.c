/*
 * E3.3.7_p4_E5E_I33_test_real_repair_resume_main.c
 *
 *  Created on: Aug 25, 2026
 *      Author: Pedro Marques
 * 
 * E3.3.7 Phase 4 / P4-E5E I33
 * End-to-end E5E -> E5D -> Reliability repair/resume validation (I33 gate).
 *
 * Scope:
...
#define I33_NETWORK_ID          1U
#define I33_NODE_A              1U
#define I33_NODE_B              2U
#define I33_NODE_C              3U
#define I33_ROUTE_SEQUENCE      7U
#define I33_WAIT_MS             30000U
#define I33_NEIGHBOR_WAIT_MS    30000U
#define I33_TASK_STACK_SIZE     4096U
#define I33_TASK_PRIORITY       5U

static const char *TAG = "E3_3_7_P4_E5E_I33";
...
static enp_reliability_handle_t s_reliability_handle = ENP_RELIABILITY_INVALID_HANDLE;
static volatile enp_reliability_repair_id_t s_e5e_repair_id = ENP_RELIABILITY_INVALID_REPAIR_ID;
static volatile bool s_e5e_repair_requested;
static volatile unsigned s_e5e_repair_request_count;
static volatile enp_e5e_correlation_id_t s_first_correlation = ENP_E5E_INVALID_CORRELATION_ID;
...
static void observed_transport_send_result(
    ...
    const enp_neighbor_t *c = find_neighbor(I33_NODE_C);
...
static StackType_t s_announce_task_stack[I33_TASK_STACK_SIZE];
static StaticTask_t s_tick_task_control;
static StackType_t s_tick_task_stack[I33_TASK_STACK_SIZE];
static StaticTask_t s_control_task_control;
static StackType_t s_control_task_stack[I33_TASK_STACK_SIZE];
...
static bool wait_wifi(void) {
    ...
    while (!enp_wifi_is_connected()) {
        if ((xTaskGetTickCount() - start) >= pdMS_TO_TICKS(I33_WAIT_MS)) {
            return false;
        }
...
static const enp_neighbor_t *find_neighbor(enp_node_id_t node) {
    const enp_address_t address = {.network = I33_NETWORK_ID, .node = node};
...
static bool resolve_transport(void *context,
    ...
    (void)context;
    if (address == NULL || next_hop.network_id != I33_NETWORK_ID) {
        return false;
    }
...
static bool select_next_hop(void *context,
                            ...
                            enp_route_destination_t *next_hop) {
    (void)context;
    if (next_hop == NULL || destination.network_id != I33_NETWORK_ID) {
        return false;
    }

    /* First hardware gate: A repairs C through the direct A-C alternative. */
    if (CONFIG_ENP_E3_NODE_ID == I33_NODE_A && destination.node_id == I33_NODE_C) {
        if (failed_next_hop.node_id == I33_NODE_C) return false;
        const enp_neighbor_t *c = find_neighbor(I33_NODE_C);
        if (c != NULL && c->state == ENP_NEIGHBOR_STATE_ACTIVE) {
            *next_hop = (enp_route_destination_t){
                .network_id = I33_NETWORK_ID, .node_id = I33_NODE_C};
            return true;
        }
    }
...
static void e5e_correlation_allocated(
    void *context, enp_reliability_handle_t handle,
    enp_e5e_correlation_id_t correlation_id) {
    (void)context;
    if (handle != s_reliability_handle || correlation_id == ENP_E5E_INVALID_CORRELATION_ID) {
        return;
    }
...
static bool e5e_repair_request(void *context,
    ...
    (void)context;
    ++s_e5e_repair_request_count;
    s_e5e_repair_id = repair_id;
    s_e5e_repair_requested = true;
    ESP_LOGI(TAG,
             "E5E repair request -> E5D: destination=%u/%u failed-next-hop=%u/%u repair_id=%u",
    ...
    if (!enp_route_repair_request(&s_repair, destination, failed_next_hop)) {
        ESP_LOGE(TAG, "FAIL: E5D repair request rejected");
        return false;
    }
    return true;
}
...
static bool install_initial_route(void) {
    const enp_neighbor_t *b = find_neighbor(I33_NODE_B);
    ...
    entry.next_hop = (enp_route_destination_t){
        .network_id = I33_NETWORK_ID, .node_id = I33_NODE_B};
    entry.route_sequence = I33_ROUTE_SEQUENCE;
    ...
    for (size_t i = 0U; i < s_routes.count; ++i) {
        if (s_routes.entries[i].destination.node_id == I33_NODE_C) {
            return enp_route_table_update(&s_routes, &entry);
        }
    }
...
static bool make_data_packet(enp_packet_t *packet) {
    ...
    header->destination.network = I33_NETWORK_ID;
    header->destination.node = I33_NODE_C;
    header->flags = ENP_FLAG_ACK_REQUIRED;
    header->ttl = ENP_MAX_TTL;
    header->sequence = 0xE5E13301U;

    static const uint8_t payload[] = "ENP-P4-E5E-I33-HW";
    ...
    if (!enp_data_header_valid(&data_header)) {
        ESP_LOGE(TAG, "FAIL: generated reliable DATA header is invalid");
        return false;
    }
    ...
    return enp_packet_seal(packet, total_payload_length) == ESP_OK;
}
...
static void run_failure_and_repair(void) {
    ...
    check(before != NULL && before->next_hop.node_id == I33_NODE_B,
          "initial route C uses next-hop B");

    ESP_LOGI(TAG, "======================================");
    ESP_LOGI(TAG, "POWER OFF NODE 2 / B NOW");
    ESP_LOGI(TAG, "Then type: e5e fail");
    ESP_LOGI(TAG, "======================================");
    ...
    enp_packet_t packet = {0};
    if (!make_data_packet(&packet)) {
        ESP_LOGE(TAG, "FAIL: Reliability DATA packet construction");
        s_test_failed = true;
        return;
    }

    s_e5e_repair_requested = false;
    s_e5e_repair_id = ENP_RELIABILITY_INVALID_REPAIR_ID;
    s_e5e_repair_request_count = 0U;
    ...
    if (initial_submit_failed) {
        ESP_LOGW(TAG,
                 "Reliability DATA submission returned false; "
                 "classifying synchronous initial-submit failure and "
                 "continuing E5E repair observation");

        if (s_reliability_handle != ENP_RELIABILITY_INVALID_HANDLE) {
            ESP_LOGI(TAG,
                     "Initial submit failure retained Reliability handle H=%u",
                     (unsigned)s_reliability_handle);
        } else {
            ESP_LOGW(TAG,
                     "Initial submit failure exposed no Reliability handle; "
                     "continuing with transport/E5E observation only");
        }
    } else {
        check(s_reliability_handle != ENP_RELIABILITY_INVALID_HANDLE,
              "real Reliability transaction created");
    }
    enp_e5e_correlation_id_t first_correlation = ENP_E5E_INVALID_CORRELATION_ID;
    if (s_reliability_handle != ENP_RELIABILITY_INVALID_HANDLE) {
        if (enp_e5e_get_correlation(s_reliability_handle, &first_correlation)) {
            s_first_correlation = first_correlation;
            ESP_LOGI(TAG,
                     "Initial E5E correlation observed: H=%u C=0x%016llX",
                     (unsigned)s_reliability_handle,
                     (unsigned long long)first_correlation);
        } else {
            ESP_LOGW(TAG,
                     "Initial E5E correlation not yet available; "
                     "continuing observation");
        }
    } else {
        ESP_LOGW(TAG,
                 "No Reliability handle after initial submit; "
                 "correlation association is not assertable at this stage");
    }

    const TickType_t failure_start = xTaskGetTickCount();
    while (!s_e5e_repair_requested &&
           (xTaskGetTickCount() - failure_start) < pdMS_TO_TICKS(I33_WAIT_MS)) {
        /* Observe E5E asynchronously; do not manufacture Reliability retries. */
        vTaskDelay(pdMS_TO_TICKS(20U));
    }

    check(s_e5e_repair_requested,
          "real ESP-NOW TX failure reached E5E repair boundary");
    check(s_e5e_repair_request_count == 1U,
    ...
    if (s_reliability_handle != ENP_RELIABILITY_INVALID_HANDLE) {
        ...
        ESP_LOGW(TAG,
                 ...
                 "applicable: no surviving Reliability handle");
    }
    ...
    bool resumed = false;
    while (!failed &&
           (xTaskGetTickCount() - resume_start) < pdMS_TO_TICKS(2000U)) {
        enp_reliability_tick(enp_context_time_ms(&s_context));
        if (enp_reliability_get_state(s_reliability_handle, &state) &&
    ...
    if (!failed) {
        ESP_LOGI(TAG, "--------------------------------------");
        ESP_LOGI(TAG, "E3.3.7 Phase 4 / P4-E5E I33 hardware validation PASS");
        ESP_LOGI(TAG, "======================================");
    } else {
        ESP_LOGE(TAG, "--------------------------------------");
        ESP_LOGE(TAG, "E3.3.7 Phase 4 / P4-E5E I33 hardware validation FAIL");
        ESP_LOGE(TAG, "======================================");
    }
}
...
static void control_task(void *arg) {
    ...
    char line[32];
    for (;;) {
        ESP_LOGI(TAG, "Commands: e5e fail | e5e routes");
        if (fgets(line, sizeof(line), stdin) == NULL) {
            vTaskDelay(pdMS_TO_TICKS(500U));
            continue;
        }
        if (strncmp(line, "e5e fail", 8U) == 0) {
            s_fail_command = true;
            vTaskDelete(NULL);
            return;
        }
        if (strncmp(line, "e5e routes", 10U) == 0) {
            const enp_route_entry_t *route = enp_route_table_lookup_const(
...
void app_main(void) {
    ESP_LOGI(TAG, "======================================");
    ESP_LOGI(TAG, "E3.3.7 PHASE 4 / P4-E5E I33");
    ESP_LOGI(TAG, "REAL E5E -> E5D -> Reliability RESUME");
    ESP_LOGI(TAG, "Real Reliability -> E5E -> E5D -> R4 -> Reliability");
    ESP_LOGI(TAG, "Target: ESP-IDF 6.0.2");
    ESP_LOGI(TAG, "Real E5D Step-3 adapter; frozen source unchanged");
    ...
    if (!enp_reliability_init() || !enp_reliability_start()) {
        ESP_LOGE(TAG, "FAIL: Reliability initialization/start");
        return;
    }
    if (!enp_e5e_integration_init(&s_data_path, e5e_repair_request, NULL) ||
        !enp_e5e_set_correlation_allocated_observer(
            e5e_correlation_allocated, NULL)) {
    ...
    (void)xTaskCreateStatic(announce_task, "i33_announce", I33_TASK_STACK_SIZE,
                            NULL, I33_TASK_PRIORITY, s_announce_task_stack,
                            &s_announce_task_control);
    (void)xTaskCreateStatic(tick_task, "i33_tick", I33_TASK_STACK_SIZE,
                            NULL, I33_TASK_PRIORITY, s_tick_task_stack,
                            &s_tick_task_control);
    (void)xTaskCreateStatic(control_task, "i33_control", I33_TASK_STACK_SIZE,
                            NULL, I33_TASK_PRIORITY, s_control_task_stack,
                            &s_control_task_control);

    if (CONFIG_ENP_E3_NODE_ID == I33_NODE_A) {
        ESP_LOGI(TAG, "Waiting for real neighbors B and C...");
        ESP_LOGI(TAG, "Neighbor discovery timeout: %u ms",
                 (unsigned)I33_NEIGHBOR_WAIT_MS);

        const TickType_t start = xTaskGetTickCount();
        TickType_t last_status = start;

        while ((xTaskGetTickCount() - start) <
               pdMS_TO_TICKS(I33_NEIGHBOR_WAIT_MS)) {
            const bool b_ready =
                find_neighbor(I33_NODE_B) != NULL &&
                find_neighbor(I33_NODE_B)->state == ENP_NEIGHBOR_STATE_ACTIVE;
            const bool c_ready =
                find_neighbor(I33_NODE_C) != NULL &&
                find_neighbor(I33_NODE_C)->state == ENP_NEIGHBOR_STATE_ACTIVE;
        ...
        ESP_LOGI(TAG, "READY: node participates in P4-E5E I33 hardware validation");
        for (;;) vTaskDelay(pdMS_TO_TICKS(1000U));
    }
}
