/*
 * E3.3.7_p4_E5D_hardware_validation_main_v2.c
 *
...
static uint32_t s_rreq_tx_observed;
static uint32_t s_rrep_tx_observed;
...
static void route_failure(void *context,
    ...
    if (!enp_route_repair_request(&s_repair, destination, failed_next_hop)) {
        ...
        ESP_LOGI(TAG,
                 "E5D request accepted: destination=%u failed-next-hop=%u",
                 (unsigned)destination.node_id,
                 (unsigned)failed_next_hop.node_id);
    }
}
...
static void process_rreq(const enp_packet_t *packet,
    ...
    const enp_routing_rreq_t *wire_rreq =
        (const enp_routing_rreq_t *)enp_packet_payload_const(packet);
    ESP_LOGI(TAG,
             "RREQ RX raw: src=%u dst=%u id=%lu hop=%u ttl=%u seq=%lu",
             (unsigned)header->source.node,
             (unsigned)wire_rreq->destination_node_id,
             (unsigned long)wire_rreq->route_request_id,
             (unsigned)wire_rreq->hop_count,
             (unsigned)wire_rreq->ttl,
             (unsigned long)header->sequence);

    enp_address_t immediate = {0};
    if (!find_neighbor_logical(source, &immediate)) {
        ESP_LOGW(TAG,
                 "RREQ RX rejected: immediate sender MAC is not a known active neighbor");
        log_neighbors();
        return;
    }
    ...
    ++s_rreq_rx_count;
    ESP_LOGI(TAG, "RREQ RX: from=%u destination=%u immediate=%u result=%d",
             (unsigned)header->source.node,
             (unsigned)rreq.destination_node_id,
             (unsigned)immediate.node, (int)result);

    if (result == ENP_RREQ_RESULT_REPLY) {
        ESP_LOGI(TAG,
                 "RREQ destination reached: generated RREP toward immediate sender %u",
                 (unsigned)immediate.node);
        ++s_rrep_tx_observed;
    }
}
...
static void run_failure_and_repair(void) {
    ...
    bool repaired = false;
    uint32_t last_rreq_tx_count = s_adapter.rreq_tx_count;

    while ((xTaskGetTickCount() - start) < pdMS_TO_TICKS(E5D_WAIT_MS)) {
        if (s_adapter.rreq_tx_count != last_rreq_tx_count) {
            last_rreq_tx_count = s_adapter.rreq_tx_count;
            s_rreq_tx_observed = last_rreq_tx_count;
            ESP_LOGI(TAG,
                     "RREQ TX observed: count=%lu destination=%u failed-next-hop=%u",
                     (unsigned long)s_adapter.rreq_tx_count,
                     (unsigned)s_adapter.active_request.destination.node_id,
                     (unsigned)s_adapter.active_request.failed_next_hop.node_id);
        }
    ...
    check(s_adapter.rreq_tx_count > 0U,
          "Step-3 adapter submitted at least one real RREQ");
    if (s_adapter.rreq_tx_count == 0U) {
        ESP_LOGE(TAG,
                 "RREQ was not submitted by the adapter; inspect E5D next-hop selection/transport resolution");
    }
    ...
    ESP_LOGI(TAG,
             "RREQ TX count=%lu RREQ RX count=%lu RREP TX observed=%lu RREP RX count=%lu DATA RX count=%lu",
             (unsigned long)s_adapter.rreq_tx_count,
             (unsigned long)s_rreq_rx_count,
             (unsigned long)s_rrep_tx_observed,
