/*
 * E3.3.7_p4_E5D_hardware_validation_main_v3.2_GW_RX_DIAG.c
 *
...
static void receive_callback(const enp_transport_address_t *source,
    ...
    ESP_LOGI(TAG,
             "DIAG RX: raw frame len=%u from=%02X:%02X:%02X:%02X:%02X:%02X",
             (unsigned)length,
             source->mac[0], source->mac[1], source->mac[2],
             source->mac[3], source->mac[4], source->mac[5]);

    if (!enp_packet_verify(&packet)) {
        ++s_rx_verify_fail_count;
        ESP_LOGE(TAG,
                 "DIAG RX DROP: enp_packet_verify() failed "
                 "count=%lu from=%02X:%02X:%02X:%02X:%02X:%02X",
                 (unsigned long)s_rx_verify_fail_count,
                 source->mac[0], source->mac[1], source->mac[2],
                 source->mac[3], source->mac[4], source->mac[5]);
        return;
    }
    ...
    const enp_header_t *header = enp_packet_header_const(&packet);
    if (header == NULL) {
        ESP_LOGE(TAG, "DIAG RX DROP: NULL packet header");
        return;
    }

    ESP_LOGI(TAG,
             "DIAG RX: verified type=%u src=%u/%u dst=%u/%u seq=%lu "
             "payload_len=%u",
    ...
    if (header->type == ENP_PACKET_ROUTE) {
        ++s_rx_route_classified_count;
        const uint8_t *route_payload =
            (const uint8_t *)enp_packet_payload_const(&packet);
        const uint8_t subtype =
            route_payload != NULL ? route_payload[1] : 0xFFU;

        ESP_LOGI(TAG,
                 "DIAG RX: ROUTE classified #%lu payload_len=%u subtype=%u "
                 "(RREQ=%u RREP=%u)",
                 (unsigned long)s_rx_route_classified_count,
                 (unsigned)header->payload_length,
                 (unsigned)subtype,
    ...
    if (header->type == ENP_PACKET_ROUTE &&
        ...
        if (payload != NULL && payload[1] == ENP_ROUTING_SUBTYPE_RREP) {
            ++s_rx_rrep_classified_count;
            ESP_LOGI(TAG,
                     "DIAG RX: RREP classified #%lu -> process_rrep()",
                     (unsigned long)s_rx_rrep_classified_count);
            process_rrep(&packet, source);
        }
        return;
        }
        ESP_LOGW(TAG,
                 "DIAG RX: ROUTE packet did not match RREQ/RREP wire-size "
                 "classification");
        return;
    }
...
static void run_failure_and_repair(void) {
    ...
    ESP_LOGI(TAG,
             "DIAG RX summary: verify_fail=%lu route=%lu rreq=%lu rrep=%lu",
             (unsigned long)s_rx_verify_fail_count,
             (unsigned long)s_rx_route_classified_count,
             (unsigned long)s_rx_rreq_classified_count,
             (unsigned long)s_rx_rrep_classified_count);
    ESP_LOGI(TAG,
...
void app_main(void) {
    ...
    ESP_LOGI(TAG, "STEP-3 HARDWARE VALIDATION V3.2 GATEWAY RX DIAGNOSTIC");
