...
static void mock_receive_callback(const enp_transport_address_t *source,
...
static esp_err_t mock_transport_send(const enp_transport_address_t *destination,
									 const void *data, size_t length) {
...
static esp_err_t
mock_transport_set_receive_callback(enp_transport_receive_callback_t callback) {
	if (callback == NULL) {
		return ESP_ERR_INVALID_ARG;
	}
...
static bool install_route(uint32_t next_hop, uint32_t route_sequence) {
...
static bool install_neighbor(uint32_t node_id, uint8_t transport_seed) {
	...
	return enp_neighbor_update(&s_context.neighbors, &address,
							   &transport_address, ENP_ROLE_RELAY, 0U, 1U, 0,
							   0U) == ESP_OK;
}

static bool make_data_packet(enp_packet_t *packet, enp_sequence_t data_sequence,
	...
	static const uint8_t payload[] = {0xE3U, 0x37U, 0x03U, 0x01U};
...
static bool make_ack_packet(enp_packet_t *packet, enp_sequence_t data_sequence,
							uint32_t application_sequence,
...
static esp_err_t reliability_submit_to_routing(const enp_packet_t *packet,
											   void *user_context) {
	enp_routing_data_path_t *path = (enp_routing_data_path_t *)user_context;
...
static bool packets_equal(const enp_packet_t *left, const enp_packet_t *right) {
	if ((left == NULL) || (right == NULL)) {
		return false;
	}

	return memcmp(enp_packet_data_const(left), enp_packet_data_const(right),
...
static bool check_result(bool condition, const char *message, bool *pass) {
	if (condition) {
...
void app_main(void) {
	...
	check_result(enp_context_init(&s_context, &s_mock_transport, &config) ==
					 ESP_OK,
				 "ENP context initialized", &pass);

	check_result(s_transport_init_calls == 1U,
				 "controlled transport initialized once", &pass);

	check_result(enp_transport_set_receive_callback(
					 &s_mock_transport, mock_receive_callback) == ESP_OK &&
					 s_transport_callback_calls == 1U &&
					 s_receive_callback == mock_receive_callback,
				 "controlled transport receive callback registered", &pass);

	check_result(enp_route_table_init(&s_routes), "route table initialized",
				 &pass);

	check_result(install_neighbor(TEST_NEXT_HOP_B, 0x20U) &&
					 install_neighbor(TEST_NEXT_HOP_E, 0x50U),
				 "real context neighbor mappings installed", &pass);

	check_result(install_route(TEST_NEXT_HOP_B, 1U),
				 "active route to C initially selects B", &pass);

	check_result(enp_routing_data_path_init(
					 &s_routing_data_path, &s_routes, s_context.transport,
	 ...
	 */
	check_result(enp_reliability_init(), "reliability initialized", &pass);

	check_result(enp_reliability_start(), "reliability started", &pass);

	check_result(enp_reliability_set_submit_callback(
					 reliability_submit_to_routing, &s_routing_data_path),
				 "reliability submit callback connected to routing data path",
				 &pass);

	check_result(enp_reliability_set_result_callback(reliability_result, NULL),
				 "reliability result callback configured", &pass);

	check_result(enp_dispatcher_init(&s_context) == ESP_OK,
				 "dispatcher initialized", &pass);

	check_result(enp_dispatcher_register(enp_reliability_service_get()) ==
					 ESP_OK,
				 "reliability ACK service registered", &pass);

	check_result(
		make_data_packet(&s_packet, TEST_DATA_SEQUENCE, TEST_APP_SEQUENCE),
		"reliable DATA packet constructed", &pass);

	enp_reliability_handle_t handle = ENP_RELIABILITY_INVALID_HANDLE;

	const unsigned sends_before = s_transport_send_calls;

	check_result(enp_reliability_send(&s_packet, 1000U, &handle),
				 "reliability submitted initial DATA through routing", &pass);

	check_result(s_transport_send_calls == sends_before + 1U,
				 "initial DATA reached controlled transport", &pass);

	const enp_transport_address_t expected_b = make_transport_address(0x20U);

	check_result(transport_address_equal(&s_last_destination, &expected_b),
				 "initial route selected B transport address", &pass);

	check_result(packets_equal(&s_first_submission, &s_packet),
				 "initial submission preserved DATA packet identity", &pass);
	 ...
	 */
	check_result(install_route(TEST_NEXT_HOP_E, 2U),
				 "active route updated to select E", &pass);

	enp_reliability_tick(2000U);

	check_result(s_transport_send_calls == sends_before + 2U,
				 "retransmission reached controlled transport", &pass);

	const enp_transport_address_t expected_e = make_transport_address(0x50U);

	check_result(transport_address_equal(&s_last_destination, &expected_e),
				 "retransmission re-ran routing and selected E", &pass);

	check_result(packets_equal(&s_retry_submission, &s_first_submission),
				 "retransmission preserved DATA transaction identity", &pass);

	check_result(make_ack_packet(&s_ack_packet, TEST_DATA_SEQUENCE,
								 TEST_APP_SEQUENCE, TEST_ACK_SEQUENCE),
				 "ACK packet constructed", &pass);

	const enp_transport_address_t ack_source = make_transport_address(0x70U);

	check_result(enp_dispatcher_dispatch(&s_ack_packet, &ack_source) == ESP_OK,
				 "ACK passed through dispatcher to reliability service", &pass);

	check_result(s_result_count == 1U &&
					 s_last_result == ENP_RELIABILITY_RESULT_DELIVERED,
				 "reliability transaction completed as DELIVERED", &pass);

	/* A second ACK must not complete the transaction again. */
	check_result(enp_dispatcher_dispatch(&s_ack_packet, &ack_source) == ESP_OK,
				 "duplicate ACK safely handled by dispatcher", &pass);

	check_result(s_result_count == 1U,
				 "duplicate ACK did not complete transaction twice", &pass);

	check_result(enp_dispatcher_deinit() == ESP_OK, "dispatcher deinitialized",
				 &pass);

	enp_reliability_deinit();

	check_result(enp_context_deinit(&s_context) == ESP_OK,
				 "ENP context deinitialized", &pass);

	check_result(s_transport_deinit_calls == 1U,
				 "controlled transport deinitialized once", &pass);

	ESP_LOGI(TAG, "--------------------------------------");

	if (pass) {
		ESP_LOGI(TAG, "E3.3.7 Phase 3 / E3 self-test PASS");
	} else {
		ESP_LOGE(TAG, "E3.3.7 Phase 3 / E3 self-test FAIL");
	}
