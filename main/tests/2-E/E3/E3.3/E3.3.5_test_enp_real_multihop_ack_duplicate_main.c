/*
 * E3.3.5_test_enp_real_multihop_ack_duplicate_main_RELAY_ROLE.c
 *
 ...
 * A
 *	Starts the test.
 *	Validates only what A can observe: exactly one ACK received during the
 *complete test window. No longer checks B or C counters.
 * B
...
static void process_data(const enp_packet_t *packet,
	...
	if (header->destination.node == CONFIG_ENP_E3_NODE_ID) {
		...
		if (s_ack_duplicate_test_active && CONFIG_ENP_E3_NODE_ID == E3_NODE_C) {
			if (header->source.node != E3_NODE_A ||
				data_header.application_sequence != E3_ACK_DUP_APP_SEQ) {
				ESP_LOGE(TAG,
