...
static esp_err_t test_service_process(enp_context_t *context,
...
static bool make_packet(enp_packet_t *packet, enp_address_t source,
						enp_address_t destination, enp_sequence_t sequence) {
	if (packet == NULL) {
		return false;
	}
...
void app_main(void) {
	...
	if (!make_packet(&packet, source_address, destination_address,
					 TEST_SEQUENCE)) {
	...
	if (enp_duplicate_cache_init(&s_test_duplicate_cache) != ESP_OK) {
		...
		if (enp_duplicate_check_and_record(&s_test_duplicate_cache,
										   &source_address, TEST_SEQUENCE,
										   now_ms, &duplicate) != ESP_OK ||
			duplicate) {
			ESP_LOGE(TAG,
					 "FAIL: diagnostic duplicate cache first check unexpected");
			pass = false;
		} else if (enp_duplicate_check_and_record(
					   &s_test_duplicate_cache, &source_address, TEST_SEQUENCE,
					   enp_context_time_ms(&context), &duplicate) != ESP_OK ||
				   !duplicate) {
			ESP_LOGE(TAG, "FAIL: diagnostic duplicate cache did not suppress "
						  "second identity");
			pass = false;
		} else {
			ESP_LOGI(TAG, "PASS: duplicate-cache module suppresses identical "
						  "source+sequence");
		}
	...
	if (enp_dispatcher_dispatch_local(&packet, &source_transport) != ESP_OK) {
	...
	if ((s_last_sequence != TEST_SEQUENCE) ||
		...
		(memcmp(s_last_source.value, source_transport.value,
				source_transport.length) != 0)) {
		ESP_LOGE(
			TAG,
			"FAIL: local dispatch did not preserve packet/source identity");
		pass = false;
	} else {
	...
	ESP_LOGI(TAG,
			 ...
			 (unsigned long)source_address.node, (unsigned long)TEST_SEQUENCE);

	if (enp_dispatcher_dispatch(&packet, &source_transport) != ESP_OK) {
		ESP_LOGE(TAG, "FAIL: normal dispatcher dispatch returned error");
		pass = false;
	} else if (s_service_calls != 3U) {
		ESP_LOGE(
			TAG,
			"FAIL: normal dispatcher first dispatch did not reach service");
		pass = false;
	} else {
	...
	ESP_LOGI(TAG,
			 "DIAG: normal dispatch #2 source=network=%u node=%lu seq=0x%08lX "
			 "service_calls_before=%lu",
			 (unsigned)source_address.network,
			 (unsigned long)source_address.node, (unsigned long)TEST_SEQUENCE,
	...
	if (enp_dispatcher_dispatch(&packet, &source_transport) != ESP_OK) {
		ESP_LOGE(TAG,
				 "FAIL: normal dispatcher duplicate dispatch returned error");
		pass = false;
	} else if (s_service_calls == 3U) {
		ESP_LOGI(TAG, "PASS: normal dispatcher generic duplicate cache "
