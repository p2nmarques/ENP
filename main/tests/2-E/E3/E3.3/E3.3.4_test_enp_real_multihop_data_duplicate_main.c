/*
 * E3.3.4_test_enp_real_multihop_data_duplicate_main_RELAY_ROLE.c
 *
 *  Created on: Aug 15, 2026
 *      Author: Pedro Marques
 *
 * ENP v0.2 — E3.3.4 real three-node DATA duplicate suppression.
 *
 ...
 * discovery timeout.
 *
 * E3.3.4 reuses the E3.2.2 route discovery and E3.3.2 DATA implementation.
 ...
 *
 * 3.3.4 test sequence:
 *  1. A creates DATA seq=0x2101, app_seq=1 and transmits the exact same
 *     sealed packet twice.
 *  2. B must forward the first copy and suppress the second copy.
 *  3. C must deliver the first packet exactly once and generate one ACK.
 *  4. A creates a new DATA seq=0x2102, app_seq=2 and sends it once.
 *  5. B must forward it and C must deliver it.
 *  6. A must receive exactly two ACKs, one for each unique DATA packet.
 *
 * Expected key messages:
 *  B: PASS: B suppressed duplicate DATA packet
 *  B: PASS: B forwarded exactly 2 unique DATA packets
 *  C: PASS: C unique DATA delivery count=1
 *  C: PASS: C unique DATA delivery count=2
 *  A: PASS: duplicate-test ACK 1 matches DATA sequence/app sequence
 *  A: PASS: duplicate-test ACK 2 matches DATA sequence/app sequence
 *  A: PASS: A received exactly 2 ACKs for unique DATA packets
 *  A: ALL E3.3.4 DATA DUPLICATE TESTS PASSED
 *
 * The test does not modify the production ESP-NOW transport.
 * All test tasks remain statically allocated.
 */
...
/* E3.3.4 duplicate-suppression test state. */
...
static void start_duplicate_test(void) {
	if (CONFIG_ENP_E3_NODE_ID != E3_NODE_A) {
		ESP_LOGW(TAG, "Only node A (node 1) can start E3.3.4");
		return;
	}
	...
	if (route == NULL || route->state != ENP_ROUTE_STATE_ACTIVE ||
		...
		ESP_LOGE(TAG, "FAIL: E3.3.4 requires an active A -> B -> C route");
		return;
	}
	...
	ESP_LOGI(TAG, "======================================");
	ESP_LOGI(TAG, "E3.3.4 START: DATA duplicate suppression");
	...
	if (!s_test_failed) {
		ESP_LOGI(TAG, "======================================");
		ESP_LOGI(TAG, "ALL E3.3.4 DATA DUPLICATE TESTS PASSED");
		ESP_LOGI(TAG, "======================================");
	}
...
static esp_err_t send_ack_from_c(enp_sequence_t data_sequence,
	...
	if (err == ESP_OK) {
		ESP_LOGI(TAG,
				 "ACK TX: origin=C destination=A next-hop=B seq=%lu "
				 "data_seq=%lu app_seq=%lu ttl=%u",
				 (unsigned long)header->sequence,
				 (unsigned long)ack.data_packet_sequence,
				 (unsigned long)ack.application_sequence,
				 (unsigned)header->ttl);
		ESP_LOGI(TAG, "PASS: C submitted ACK to B");
	}
...
static void process_data(const enp_packet_t *packet,
	...
	if (header->destination.node == CONFIG_ENP_E3_NODE_ID) {
		...
		if (send_ack_from_c(header->sequence,
							data_header.application_sequence) != ESP_OK) {
...
static void control_task(void *argument) {
	...
	ESP_LOGI(TAG, "READY");
	ESP_LOGI(TAG, "Waiting for E3.2.2 / E3.3.4 commands");
	...
	for (;;) {
		ESP_LOGI(TAG, "Type 'e3 start' for route discovery, 'e3 data' or 'e3 "
					  "dup' on node A");
		...
		if (strncmp(line, "e3 start", 8U) == 0) {
			...
			ESP_LOGI(TAG, "Commands: e3 start | e3 data | e3 dup | e3 routes");
		}
	}
}
...
void app_main(void) {
	...
	ESP_LOGI(TAG, "======================================");
	ESP_LOGI(TAG, "ENP v0.2 E3.3.4 REAL MULTI-HOP DATA DUPLICATE SUPPRESSION");
	ESP_LOGI(TAG, "======================================");
