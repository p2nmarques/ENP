 ...
 *## Important
 *
 * `ENP_PACKET_APPLICATION` is the existing canonical ENP packet type. E3.3.2
 *does **not** add or renumber an ENP packet type.
 *
 *The E3.3.1 DATA header is carried inside `ENP_PACKET_APPLICATION`.
 *
 *## Test sequence
 *
 *1. Flash the same E3.3.2 test application to A, B and C with the existing
 ...
 *`A -> B`
 *
 *B resolves destination C using its route table and forwards the same packet
 *with TTL decremented:
 *
 *`B -> C`
 *
 *C validates and delivers the application payload.
 *
 *T*he ENP source remains A and the ENP packet sequence remains unchanged during
 *forwarding. The TTL is decremented and the frame is resealed before
 *transmission.
 *
 *## Expected logs
 *
 ...
 *
 *E3.3.2 deliberately does not send an acknowledgement back to A. That belongs
 *to E3.3.3.
 */
...
static void enp_receive_callback(const enp_transport_address_t *source,
	...
	if (header->type == ENP_PACKET_DISCOVERY) {
		...
		(void)enp_neighbor_update(&s_context.neighbors, &header->source, source,
								  (enp_role_t)discovery->role,
								  discovery->capabilities, header->sequence, 0,
								  enp_context_time_ms(&s_context));

		ESP_LOGI(TAG, "Neighbor ready: node=%lu transport-len=%u",
				 (unsigned long)header->source.node, (unsigned)source->length);
		return;
	}
...
static void start_data_test(void) {
	...
	if (route == NULL || route->state != ENP_ROUTE_STATE_ACTIVE) {
		ESP_LOGE(
			TAG,
			"FAIL: E3.3.2 requires an active A -> C route; run e3 start first");
		return;
	}
