 ...
 */
         ...
         size_t length)
 {
     ...
     if ((destination == NULL) || (data == NULL) ||
         (length == 0U) || (length > sizeof(s_tx_buffer)))
     {
 ...
 static esp_err_t mock_set_receive_callback(
 ...
 static void test_receive_callback(
 ...
 void app_main(void)
 {
     ...
     if (s_init_calls != 1U)
     {
         ESP_LOGE(TAG, "FAIL: transport init callback expected once, got %u", s_init_calls);
         pass = false;
     }
     else
     {
     ...
     if (enp_transport_set_receive_callback(
             &s_mock_transport, test_receive_callback) != ESP_OK)
     {
         ESP_LOGE(TAG, "FAIL: controlled transport receive callback registration");
         pass = false;
     }
     else
     {
     ...
     const enp_address_t next_hop_address = make_logical_address(TEST_NEXT_HOP_NODE_ID);
     const enp_transport_address_t next_hop_transport = make_transport_address(0x20U);
     ...
     if (enp_neighbor_get_transport_address(
             &s_context.neighbors, &next_hop_address, &resolved_transport) != ESP_OK)
     {
         ESP_LOGE(TAG, "FAIL: logical neighbor lookup");
         pass = false;
     }
     else if (!transport_address_equal(&resolved_transport, &next_hop_transport))
     {
         ESP_LOGE(TAG, "FAIL: logical neighbor resolved to wrong transport address");
         pass = false;
     }
     else
     {
         ESP_LOGI(TAG, "PASS: logical neighbor resolves to stored transport address");
     }
     ...
     {
         ESP_LOGI(TAG, "PASS: routing data path initialized with real context resolver");
     }
     ...
     if (enp_routing_data_path_submit(&s_routing_data_path, &s_packet) != ESP_OK)
     {
     ...
     {
         ESP_LOGI(TAG, "PASS: routing data path submitted DATA through context resolver");
     }

     if (s_send_calls != 1U)
     {
         ESP_LOGE(TAG, "FAIL: controlled transport send count expected 1, got %u", s_send_calls);
         pass = false;
     }
     else
     {
     ...
     {
         ESP_LOGI(TAG, "PASS: routing path selected real neighbor transport address");
     }

     if ((s_tx_length != enp_packet_length(&s_packet)) ||
         memcmp(s_tx_buffer, enp_packet_data_const(&s_packet), s_tx_length) != 0)
     {
         ESP_LOGE(TAG, "FAIL: submitted DATA changed during context integration");
         pass = false;
     }
     else
     {
     ...
     if ((header == NULL) || (header->ttl != TEST_TTL) ||
         ...
         (header->flags != ENP_FLAG_ACK_REQUIRED))
     {
         ESP_LOGE(TAG, "FAIL: DATA identity/TTL changed during origin submission");
         pass = false;
     }
     else
     {
     ...
     {
         ESP_LOGE(TAG, "FAIL: transport deinit callback expected once, got %u", s_deinit_calls);
         pass = false;
     }
     else
     {
