 ...
 *
 * ENP v0.2 — E3.3.3 real three-node DATA ACK C -> B -> A.
 *
 ...
 * discovery timeout.
 *
 * E3.3.3 reuses the E3.2.2 route discovery and E3.3.2 DATA implementation.
 * The ACK uses the dedicated ENP ACK payload and the existing integration
 * transport; the production ESP-NOW transport is not modified.
 */
...
static esp_err_t send_data_from_a(void) {
	...
	/* Retain the exact DATA identity so A can validate the returning ACK. */
	s_last_data_packet_sequence = header->sequence;
	...
	enp_data_header_init(&data_header, ENP_DATA_SUBTYPE_APPLICATION,
						 ENP_DATA_FLAG_NONE, s_data_application_sequence,
						 payload_length);
	...
	esp_err_t err = enp_packet_seal(&packet, total_payload_length);
	if (err != ESP_OK) {
...
static void start_data_test(void) {
	...
	ESP_LOGI(TAG, "======================================");
	ESP_LOGI(TAG, "E3.3.3 START: A -> B -> C DATA + C -> B -> A ACK");
...
static void control_task(void *argument) {
	...
	ESP_LOGI(TAG, "READY");
	ESP_LOGI(TAG, "Waiting for E3.2.2 / E3.3.3 commands");
	...
	for (;;) {
		ESP_LOGI(
			TAG,
			"Type 'e3 start' for route discovery, then 'e3 data' on node A");
...
void app_main(void) {
	...
	ESP_LOGI(TAG, "======================================");
	ESP_LOGI(TAG, "ENP v0.2 E3.3.3 REAL MULTI-HOP DATA + ACK");
	ESP_LOGI(TAG, "======================================");
