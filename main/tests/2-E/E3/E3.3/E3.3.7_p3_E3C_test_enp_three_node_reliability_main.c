 ...
 static void process_b_packet(void)
 {
     ...
     if (header->type == (uint8_t)ENP_PACKET_APPLICATION)
     {
         ...
         if (duplicate)
         {
             ...
             ESP_LOGI(TAG,
                      ...
                      (uint32_t)header->sequence);

             if (s_cached_ack_valid)
             {
                 if (enp_routing_data_path_forward(
                             &s_routing_path,
                             &s_cached_ack) != ESP_OK)
                 {
                 ...
                 ESP_LOGI(TAG, "PASS: B forwarded cached ACK after duplicate DATA");
             }
             else
             {
     ...
     if (header->type == (uint8_t)ENP_PACKET_ACK)
     {
         ...
         if (!s_cached_ack_valid)
         {
             ...
             ++s_b_ack_drop_count;
             ESP_LOGI(TAG,
                      "TEST: B deliberately drops first ACK and caches it");
             return;
         }
