/*
 * E3.3.7_p3_E3C_test_enp_three_node_reliability_main.c
 *
 *  Created on: Aug 16, 2026
 *      Author: Pedro Marques
 *
 * E3.3.7 Phase 3 / E3C — three-node end-to-end reliability integration.
 *
 * Topology:
 *
...
static const char *TAG = "E3_3_7_E3C";
...
static const uint8_t E3_APP_PAYLOAD[E3_APP_PAYLOAD_LEN] = {0xE3U, 0xC3U, 0x00U,
														   0x01U};
...
static uint8_t s_rx_queue_storage[E3C_RX_QUEUE_LENGTH * sizeof(e3c_rx_item_t)];
static QueueHandle_t s_rx_queue;
static e3c_rx_item_t s_rx_item;

static enp_packet_t s_cached_ack;
static bool s_cached_ack_valid;
...
static uint32_t s_b_ack_drop_count;
static uint32_t s_b_cached_ack_forward_count;
static uint32_t s_b_duplicate_ack_count;
...
static enp_transport_address_t make_transport_address(uint8_t b0, uint8_t b1,
...
static enp_transport_address_t mac_for_node(enp_node_id_t node_id) {
	switch (node_id) {
	case E3_NODE_A:
		return make_transport_address(E3_MAC_A_0, E3_MAC_A_1, E3_MAC_A_2,
									  E3_MAC_A_3, E3_MAC_A_4, E3_MAC_A_5);
	case E3_NODE_B:
		return make_transport_address(E3_MAC_B_0, E3_MAC_B_1, E3_MAC_B_2,
									  E3_MAC_B_3, E3_MAC_B_4, E3_MAC_B_5);
	case E3_NODE_C:
		return make_transport_address(E3_MAC_C_0, E3_MAC_C_1, E3_MAC_C_2,
...
static bool install_route(enp_node_id_t destination, enp_node_id_t next_hop,
						  uint16_t hops) {
...
static bool install_local_neighbors(enp_node_id_t local_node) {
	...
	if (local_node == E3_NODE_A) {
		...
		return enp_neighbor_update(&s_context.neighbors, &b, &mac,
								   ENP_ROLE_RELAY, 0U, 1U, 0, now_ms) == ESP_OK;
	}
	...
	if (local_node == E3_NODE_B) {
		...
		return enp_neighbor_update(&s_context.neighbors, &a, &mac_a,
								   ENP_ROLE_GATEWAY, 0U, 1U, 0,
								   now_ms) == ESP_OK &&
			   enp_neighbor_update(&s_context.neighbors, &c, &mac_c,
...
static esp_err_t submit_to_routing(const enp_packet_t *packet,
								   void *user_context) {
	const esp_err_t err = enp_routing_data_path_submit(
		(enp_routing_data_path_t *)user_context, packet);
...
static void reliability_result(enp_reliability_handle_t handle,
...
static bool make_reliable_data(void) {
	...
	enp_packet_init(&s_tx_data, ENP_PACKET_APPLICATION, &source);
	...
	enp_data_header_init(data_header, ENP_DATA_SUBTYPE_APPLICATION,
						 ENP_DATA_FLAG_NONE, E3_APP_SEQUENCE,
						 E3_APP_PAYLOAD_LEN);

	memcpy(((uint8_t *)data_header) + ENP_DATA_HEADER_SIZE, E3_APP_PAYLOAD,
		   sizeof(E3_APP_PAYLOAD));

	return enp_packet_seal(&s_tx_data, (uint16_t)(ENP_DATA_HEADER_SIZE +
												  sizeof(E3_APP_PAYLOAD))) ==
		   ESP_OK;
}
...
static bool make_ack_from_data(const enp_packet_t *data_packet) {
	...
	const enp_header_t *data_header = enp_packet_header_const(data_packet);
	...
	enp_packet_init(&s_cached_ack, ENP_PACKET_ACK, &source);

	enp_header_t *ack_header = enp_packet_header(&s_cached_ack);
	if (ack_header == NULL) {
		return false;
	}
	...
	enp_ack_payload_t *ack_payload =
		(enp_ack_payload_t *)enp_packet_payload(&s_cached_ack);

	if (ack_payload == NULL) {
		return false;
	}

	enp_ack_payload_init(ack_payload, data_header->sequence,
						 payload->application_sequence);

	return enp_packet_seal(&s_cached_ack, ENP_ACK_WIRE_SIZE) == ESP_OK;
}
...
static bool packet_is_expected_data(const enp_packet_t *packet) {
	...
	if (header == NULL || header->type != (uint8_t)ENP_PACKET_APPLICATION ||
		...
		header->sequence != E3_DATA_SEQUENCE ||
		(header->flags & ENP_FLAG_ACK_REQUIRED) == 0U || header->ttl == 0U ||
	...
	if (data_header == NULL || !enp_data_header_valid(data_header) ||
	...
	return memcmp(application_payload, E3_APP_PAYLOAD,
...
static void transport_receive_callback(const enp_transport_address_t *source,
									   const void *data, size_t length) {
	if (s_rx_queue == NULL || source == NULL || data == NULL || length == 0U ||
...
static void process_a_packet(void) {
	if (!receive_next_packet()) {
		return;
	}

	const enp_packet_t *packet = &s_rx_item.packet;
	const enp_transport_address_t *source = &s_rx_item.source;
...
static void process_b_packet(void) {
	if (!receive_next_packet()) {
		return;
	}
	...
	if (header == NULL || !enp_packet_verify(packet)) {
		ESP_LOGE(TAG, "FAIL: B received invalid ENP packet");
		return;
	}
	...
	if (header->type == (uint8_t)ENP_PACKET_APPLICATION) {
		...
		if (enp_duplicate_check_and_record(&s_data_duplicates, &header->source,
										   header->sequence, now_ms,
										   &duplicate) != ESP_OK) {
			ESP_LOGE(TAG, "FAIL: B DATA duplicate-cache operation");
			return;
		}
		...
		if (duplicate) {
			...
			ESP_LOGI(
				TAG,
				"PASS: B suppressed duplicate DATA source=%u seq=0x%04" PRIX32,
				(unsigned)header->source.node, (uint32_t)header->sequence);

			if (s_cached_ack_valid) {
				if (enp_routing_data_path_forward(&s_routing_path,
												  &s_cached_ack) != ESP_OK) {
					ESP_LOGE(TAG, "FAIL: B cached ACK forwarding");
					return;
				}

				++s_b_cached_ack_forward_count;
				ESP_LOGI(TAG,
						 "PASS: B forwarded cached ACK after duplicate DATA");
			} else {
				ESP_LOGE(TAG, "FAIL: B duplicate DATA has no cached ACK");
			}
			return;
		}

		if (!packet_is_expected_data(packet)) {
			ESP_LOGE(TAG, "FAIL: B DATA validation");
			return;
		}

		if (enp_routing_data_path_forward(&s_routing_path, packet) != ESP_OK) {
			ESP_LOGE(TAG, "FAIL: B DATA forwarding to C");
			return;
		}

		ESP_LOGI(TAG, "PASS: B forwarded DATA A -> C; TTL decremented");
		return;
	}

	if (header->type == (uint8_t)ENP_PACKET_ACK) {
		bool duplicate = false;
		if (enp_duplicate_check_and_record(&s_ack_duplicates, &header->source,
										   header->sequence, now_ms,
										   &duplicate) != ESP_OK) {
			ESP_LOGE(TAG, "FAIL: B ACK duplicate-cache operation");
			return;
		}

		if (duplicate) {
			++s_b_duplicate_ack_count;
			ESP_LOGI(TAG, "PASS: B suppressed duplicate ACK");
			return;
		}

		if (!packet_is_expected_ack(packet)) {
			ESP_LOGE(TAG, "FAIL: B ACK validation");
			return;
		}

		if (!s_cached_ack_valid) {
			s_cached_ack = *packet;
			s_cached_ack_valid = true;
			++s_b_ack_drop_count;
			ESP_LOGI(TAG, "TEST: B deliberately drops first ACK and caches it");
			return;
		}

		if (enp_routing_data_path_forward(&s_routing_path, packet) != ESP_OK) {
			ESP_LOGE(TAG, "FAIL: B ACK forwarding");
			return;
		}

		ESP_LOGI(TAG, "PASS: B forwarded ACK toward A");
	}
}

static void process_c_packet(void) {
	if (!receive_next_packet()) {
		return;
	}
	...
	const enp_packet_t *packet = &s_rx_item.packet;
	const enp_header_t *header = enp_packet_header_const(packet);
	if (header == NULL || !packet_is_expected_data(packet)) {
	...
	if (enp_duplicate_check_and_record(
		...
		ESP_LOGE(TAG, "FAIL: C DATA duplicate-cache operation");
		return;
	}

	if (duplicate) {
		ESP_LOGW(TAG, "C suppressed duplicate DATA");
		return;
	}
	...
	if (enp_routing_data_path_submit(&s_routing_path, &s_cached_ack) !=
		ESP_OK) {
	...
	++s_c_ack_sent_count;
	ESP_LOGI(TAG, "PASS: C submitted ACK C -> A through route via B");
}
...
static bool common_setup(enp_node_id_t node_id) {
	...
	ESP_LOGI(TAG, "Wi-Fi connected, channel=%u node=%u",
			 (unsigned)enp_wifi_get_channel(), (unsigned)node_id);
	...
	if (!install_local_neighbors(node_id) || !install_local_topology(node_id)) {
		return false;
	}
	...
	if (enp_duplicate_cache_init(&s_data_duplicates) != ESP_OK ||
		enp_duplicate_cache_init(&s_ack_duplicates) != ESP_OK) {
		return false;
	}

	s_rx_queue = xQueueCreateStatic(E3C_RX_QUEUE_LENGTH, sizeof(e3c_rx_item_t),
									s_rx_queue_storage, &s_rx_queue_buffer);
	if (s_rx_queue == NULL) {
		return false;
	}

	if (enp_transport_set_receive_callback(
			transport, transport_receive_callback) != ESP_OK) {
		return false;
	}
...
static bool sender_setup(void) {
	if (!enp_reliability_init() ||
		!enp_reliability_set_submit_callback(submit_to_routing,
											 &s_routing_path) ||
		!enp_reliability_set_result_callback(reliability_result, NULL) ||
	...
	ESP_LOGI(TAG, "PASS: A reliability transaction prepared seq=0x%04" PRIX32,
...
static void sender_run(void) {
	...
	if (!enp_reliability_send(&s_tx_data, now_ms, &s_reliability_handle)) {
	...
	if (s_reliability_submit_count != 2U) {
		ESP_LOGE(TAG,
				 "FAIL: A expected exactly 2 routing submissions (initial + 1 "
				 "retry), got %lu",
	...
	ESP_LOGI(TAG,
			 "PASS: A observed exactly one retransmission through routing");
	ESP_LOGI(TAG, "PASS: A received recovered ACK through B");
	ESP_LOGI(TAG, "E3.3.7 Phase 3 / E3C A ROLE PASS");
}
...
static void relay_run(void) {
	...
	while ((enp_context_time_ms(&s_context) - start) < E3_TEST_TIMEOUT_MS) {
		...
		if (s_b_cached_ack_forward_count >= 1U) {
			break;
		}
	...
	if (s_b_ack_drop_count != 1U) {
		ESP_LOGE(
			TAG,
			"FAIL: B expected exactly one intentionally dropped ACK, got %lu",
			(unsigned long)s_b_ack_drop_count);
		return;
	}
	...
	if (s_b_duplicate_data_count != 1U) {
		ESP_LOGE(
			TAG,
			"FAIL: B expected exactly one duplicate DATA suppression, got %lu",
	...
	if (s_b_cached_ack_forward_count != 1U) {
		ESP_LOGE(TAG,
				 "FAIL: B expected exactly one cached ACK recovery, got %lu",
				 (unsigned long)s_b_cached_ack_forward_count);
		return;
	}

	ESP_LOGI(TAG, "PASS: B suppressed retransmitted DATA exactly once");
	ESP_LOGI(TAG, "PASS: B forwarded cached ACK exactly once");
	ESP_LOGI(TAG, "E3.3.7 Phase 3 / E3C B ROLE PASS");
}
...
static void sensor_run(void) {
	...
	if (s_c_ack_sent_count != 1U) {
		ESP_LOGE(TAG, "FAIL: C expected exactly one ACK transmission, got %lu",
	...
	ESP_LOGI(TAG, "E3.3.7 Phase 3 / E3C C ROLE PASS");
}

void app_main(void) {
	const enp_node_id_t node_id = (enp_node_id_t)CONFIG_ENP_E3_NODE_ID;

	ESP_LOGI(TAG, "======================================");
	ESP_LOGI(TAG, "E3.3.7 PHASE 3 / E3C THREE-NODE RELIABILITY");
	ESP_LOGI(TAG, "A -> B -> C DATA / C -> B -> A ACK");
	ESP_LOGI(TAG, "First ACK dropped at B; retry must recover cached ACK");
	ESP_LOGI(TAG, "Target: ESP-IDF 6.0.2");
	ESP_LOGI(TAG, "Node=%u", (unsigned)node_id);
	ESP_LOGI(TAG, "======================================");

	if (!common_setup(node_id)) {
		ESP_LOGE(TAG, "E3C setup FAIL");
		return;
	}

	if (node_id == E3_NODE_A) {
		if (!sender_setup()) {
			ESP_LOGE(TAG, "E3C A setup FAIL");
			return;
		}
		sender_run();
	} else if (node_id == E3_NODE_B) {
