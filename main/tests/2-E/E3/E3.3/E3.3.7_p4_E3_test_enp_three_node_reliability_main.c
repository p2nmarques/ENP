...
/* B-side data-plane instrumentation. These counters are test observability
 * only. */
...
static esp_err_t
observed_transport_send(const enp_transport_address_t *destination,
						const void *data, size_t length) {
	if (s_underlying_transport == NULL ||
		s_underlying_transport->send == NULL || destination == NULL ||
		data == NULL || length == 0U) {
...
static bool make_ack_from_data(const enp_packet_t *data_packet) {
	...
	enp_packet_init(&s_local_ack, ENP_PACKET_ACK, &source);
	...
	return enp_packet_seal(&s_local_ack, ENP_ACK_WIRE_SIZE) == ESP_OK;
}
...
static void transport_receive_callback(const enp_transport_address_t *source,
	...
	if (s_node_id == E3_NODE_A && s_a_ack_drop_count == 0U &&
		length <= sizeof(enp_packet_t)) {
		enp_packet_t probe;
		...
		if (enp_packet_verify(&probe)) {
			const enp_header_t *probe_header = enp_packet_header_const(&probe);

			if ((probe_header != NULL) &&
				probe_header->type == (uint8_t)ENP_PACKET_ACK) {
				++s_a_ack_drop_count;
				ESP_LOGI(TAG, "TEST: A intentionally drops first ACK before "
...
static esp_err_t local_process(void *context, const enp_packet_t *packet,
	...
	if (s_node_id == E3_NODE_A && header->type == (uint8_t)ENP_PACKET_ACK) {
	...
	if (s_node_id == E3_NODE_C &&
		...
		if (enp_routing_data_path_submit(&s_routing_path, &s_local_ack) !=
			ESP_OK) {
...
static void process_next_packet(void) {
	...
	if (!enp_packet_verify(&s_rx_item.packet)) {
		ESP_LOGE(TAG, "FAIL: received invalid ENP packet (len=%u)",
				 (unsigned)enp_packet_length(&s_rx_item.packet));
		return;
	}

	const enp_header_t *header = enp_packet_header_const(&s_rx_item.packet);
	...
	if ((header->type != (uint8_t)ENP_PACKET_APPLICATION) &&
		...
		ESP_LOGI(TAG, "PASS: ignored non-DATA/ACK packet type=%u",
	...
	if (s_node_id == E3_NODE_B) {
		if (header->type == (uint8_t)ENP_PACKET_APPLICATION) {
			...
			if (s_b_data_received_count == 2U) {
				...
				ESP_LOGI(TAG, "PASS: B observed duplicate DATA; expecting "
							  "cached ACK recovery");
			}
		} else if (header->type == (uint8_t)ENP_PACKET_ACK) {
	...
	const esp_err_t err = enp_data_plane_process(
		&s_data_plane, &s_rx_item.packet, &s_rx_item.source);

	if (err != ESP_OK) {
		ESP_LOGE(TAG, "FAIL: ENP data plane rejected type=%u err=%s (%d)",
				 (unsigned)header->type, esp_err_to_name(err), (int)err);
		return;
	}

	if (s_node_id == E3_NODE_B) {
		if (header->type == (uint8_t)ENP_PACKET_APPLICATION) {
			ESP_LOGI(TAG, "PASS: B data plane processed DATA for forwarding");
		} else if (header->type == (uint8_t)ENP_PACKET_ACK) {
			ESP_LOGI(TAG,
...
static bool common_setup(enp_node_id_t node_id) {
	...
	if (!enp_routing_data_path_init(&s_routing_path, &s_routes,
									&s_observed_transport, resolve_transport,
									&s_context)) {
		return false;
	}

	if (enp_data_plane_init(&s_data_plane, &s_context, &s_routing_path,
...
static void relay_run(void) {
	...
	while ((enp_context_time_ms(&s_context) - start) < E3_TEST_TIMEOUT_MS) {
		...
		if (s_b_data_received_count >= 2U && s_b_ack_received_count >= 1U &&
	...
	if (s_b_data_received_count != 2U) {
		ESP_LOGE(TAG,
				 "FAIL: B expected 2 DATA receptions (original + duplicate), "
				 "got %lu",
	...
	if (s_b_data_forwarded_count != 1U) {
		ESP_LOGE(TAG, "FAIL: B expected 1 DATA forwarding, got %lu",
				 (unsigned long)s_b_data_forwarded_count);
		pass = false;
	}

	if (s_b_ack_received_count != 1U) {
		ESP_LOGE(TAG, "FAIL: B expected 1 ACK reception, got %lu",
	...
	if (s_b_duplicate_data_count != 1U) {
		ESP_LOGE(TAG, "FAIL: B expected 1 duplicate DATA suppression, got %lu",
	...
	if (pass) {
		...
		ESP_LOGI(TAG,
				 "PASS: B recovered and forwarded cached ACK exactly once");
...
void app_main(void) {
	...
	ESP_LOGI(TAG, "First ACK dropped before A data-plane processing; retry "
				  "must recover cached ACK at B");
