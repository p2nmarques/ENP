...
static void run_failure_and_repair(void) {
    ...
    if (!enp_reliability_send(&packet, 1000U, &s_reliability_handle)) {
    ...
    check(enp_e5e_get_correlation(s_reliability_handle, &first_correlation),
          "initial E5E correlation associated with Reliability handle");
...
static void control_task(void *arg) {
    ...
    char line[32];
    for (;;) {
        ESP_LOGI(TAG, "Commands: i33 fail | i33 routes");
        ...
        if (strncmp(line, "i33 routes", 10U) == 0) {
            const enp_route_entry_t *route = enp_route_table_lookup_const(
