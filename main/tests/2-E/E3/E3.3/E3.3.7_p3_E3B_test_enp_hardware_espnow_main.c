...
static bool make_data_packet(void) {
	...
	static const uint8_t payload[TEST_PAYLOAD_LENGTH] = {0xE3U, 0x37U, 0xE3U,
														 0x07U};
	...
	return enp_packet_seal(&s_tx_packet, (uint16_t)(ENP_DATA_HEADER_SIZE +
...
static bool make_ack_packet(void) {
	...
	enp_packet_init(&s_ack_packet, ENP_PACKET_ACK, &source);
	...
	enp_ack_payload_init(ack, TEST_SEQUENCE, TEST_APP_SEQUENCE);

	return enp_packet_seal(&s_ack_packet, ENP_ACK_WIRE_SIZE) == ESP_OK;
}
...
static void sender_receive_callback(const enp_transport_address_t *source,
									const void *data, size_t length) {
	(void)source;

	if ((data == NULL) || (length == 0U) || (length > sizeof(s_received_ack))) {
		s_ack_bad = true;
		return;
	}
...
static void receiver_receive_callback(const enp_transport_address_t *source,
									  const void *data, size_t length) {
	...
	if ((data_header == NULL) || !enp_data_header_valid(data_header) ||
		!enp_data_payload_length_valid(data_header, TEST_PAYLOAD_LENGTH) ||
	...
	const uint8_t expected_payload[TEST_PAYLOAD_LENGTH] = {0xE3U, 0x37U, 0xE3U,
														   0x07U};

	if (memcmp(application_payload, expected_payload,
	...
	if (s_data_rx_count == 1U) {
		...
		ESP_LOGI(TAG, "TEST: intentionally dropping first ACK to force retry");
		return;
	}
...
static bool sender_setup(void) {
	...
	if (enp_neighbor_update(&s_context.neighbors, &receiver, &peer,
	...
	if (!enp_routing_data_path_init(
			&s_routing_path, &s_routes, s_context.transport,
	...
	if (!enp_reliability_set_submit_callback(reliability_submit_to_routing,
											 &s_routing_path) ||
	...
	ESP_LOGI(TAG, "PASS: sender reliability connected to routing data path");
...
static bool receiver_setup(void) {
	...
	if (enp_transport_set_receive_callback(
			s_context.transport, receiver_receive_callback) != ESP_OK) {
...
static void sender_run(void) {
	...
	while ((enp_context_time_ms(&s_context) - start) < TEST_RUN_MS) {
		...
		if (s_ack_seen) {
			...
			if (!enp_packet_verify(&s_received_ack)) {
				...
				const enp_transport_address_t sender_peer =
					peer_transport_address();
				if (enp_dispatcher_dispatch(&s_received_ack, &sender_peer) !=
					ESP_OK) {
					s_ack_bad = true;
				} else {
					ESP_LOGI(
	...
	if (s_result_count != 1U ||
		s_last_result != ENP_RELIABILITY_RESULT_DELIVERED) {
		ESP_LOGE(TAG, "FAIL: reliability did not complete after real retry");
		return;
	}
...
static void receiver_run(void) {
	...
	while ((enp_context_time_ms(&s_context) - start) < TEST_RUN_MS) {
		if (s_ack_pending) {
			...
			if (enp_transport_send(
					s_context.transport, &s_pending_ack_destination,
			...
			s_ack_sent = true;
			ESP_LOGI(TAG, "PASS: real ESP-NOW ACK transmitted after retry");
		}
	...
	ESP_LOGI(TAG, "PASS: receiver observed original DATA + one retransmission");
}
...
void app_main(void) {
	...
	const bool pass = (s_result_count == 1U) &&
					  (s_last_result == ENP_RELIABILITY_RESULT_DELIVERED) &&
					  !s_ack_bad;
	...
	ESP_LOGI(TAG, "--------------------------------------");
	if (pass) {
		ESP_LOGI(TAG, "E3.3.7 Phase 3 / E3 hardware sender PASS");
	} else {
		ESP_LOGE(TAG, "E3.3.7 Phase 3 / E3 hardware sender FAIL");
	}
	ESP_LOGI(TAG, "======================================");
#else
	...
	const bool pass = s_data_seen && !s_data_bad && s_data_rx_count >= 2U &&
					  s_first_ack_dropped && s_ack_sent;

	(void)cleanup();

	ESP_LOGI(TAG, "--------------------------------------");
	if (pass) {
		ESP_LOGI(TAG, "E3.3.7 Phase 3 / E3 hardware receiver PASS");
	} else {
		ESP_LOGE(TAG, "E3.3.7 Phase 3 / E3 hardware receiver FAIL");
	}
	ESP_LOGI(TAG, "======================================");
#endif
}
