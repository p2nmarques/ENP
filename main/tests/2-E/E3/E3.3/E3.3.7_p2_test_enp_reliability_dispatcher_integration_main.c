...
static esp_err_t submit_stub(const enp_packet_t *packet, void *user_context) {
	(void)user_context;
...
static void result_callback(enp_reliability_handle_t handle,
...
static bool make_data_variant(enp_packet_t *packet,
...
static bool make_ack_variant(enp_packet_t *packet, enp_sequence_t data_sequence,
							 uint32_t application_sequence,
...
static bool make_ack(enp_packet_t *packet) {
	return make_ack_variant(packet, TEST_DATA_SEQUENCE, TEST_APP_SEQUENCE,
							TEST_ACK_SEQUENCE);
}
...
void app_main(void) {
	...
	pass &= expect(enp_dispatcher_init(&context) == ESP_OK,
				   "dispatcher initialized");

	pass &= expect(enp_reliability_init(), "reliability initialized");

	pass &= expect(enp_reliability_set_submit_callback(submit_stub, NULL),
				   "reliability submit boundary configured");

	pass &= expect(enp_reliability_set_result_callback(result_callback, NULL),
				   "reliability result callback configured");

	pass &= expect(enp_reliability_start(), "reliability started");

	pass &=
		expect(enp_dispatcher_register(enp_reliability_service_get()) == ESP_OK,
			   "reliability ACK service registered with dispatcher");

	pass &= expect(make_data(&data), "reliable DATA packet constructed");

	enp_reliability_handle_t handle = ENP_RELIABILITY_INVALID_HANDLE;

	pass &= expect(enp_reliability_send(&data, 1000U, &handle),
				   "reliable DATA submitted through reliability API");

	pass &= expect(s_submit_count == 1U,
				   "initial DATA submission count is exactly 1");

	pass &= expect(memcmp(&data, &s_last_submitted, sizeof(data)) == 0,
				   "initial submission preserves complete DATA packet");

	pass &= expect(make_ack(&ack), "correlated ACK packet constructed");

	pass &= expect(enp_dispatcher_dispatch(&ack, &transport_source) == ESP_OK,
				   "ACK entered through ENP dispatcher");

	pass &=
		expect(s_result_count == 1U &&
	 ...
	 */
	pass &=
		expect(enp_dispatcher_dispatch(&ack, &transport_source) == ESP_OK,
			   "duplicate ACK accepted by dispatcher as suppressed traffic");

	pass &= expect(s_result_count == 1U,
	...
	const uint32_t retry_application_sequence = TEST_APP_SEQUENCE + 1U;
	const enp_sequence_t retry_ack_sequence =
		(enp_sequence_t)(TEST_ACK_SEQUENCE + 1U);

	pass &= expect(make_data_variant(&retry_data, retry_data_sequence,
									 retry_application_sequence),
				   "second reliable DATA packet constructed with a new "
				   "transaction identity");

	enp_reliability_handle_t retry_handle = ENP_RELIABILITY_INVALID_HANDLE;

	pass &= expect(enp_reliability_send(&retry_data, 5000U, &retry_handle),
				   "second reliable DATA transaction created");

	enp_reliability_tick(5999U);

	pass &= expect(s_submit_count == 1U, "no retry before ACK timeout");

	enp_reliability_tick(6000U);

	pass &=
		expect(s_submit_count == 2U,
			   "reliability layer retransmitted exactly once after timeout");

	pass &=
		expect(memcmp(&retry_data, &s_last_submitted, sizeof(retry_data)) == 0,
			   "retransmission preserves identical DATA transaction identity");

	pass &= expect(
		make_ack_variant(&retry_ack, retry_data_sequence,
						 retry_application_sequence, retry_ack_sequence),
		"retry transaction correlated ACK constructed with a new ACK identity");

	pass &=
		expect(enp_dispatcher_dispatch(&retry_ack, &transport_source) == ESP_OK,
