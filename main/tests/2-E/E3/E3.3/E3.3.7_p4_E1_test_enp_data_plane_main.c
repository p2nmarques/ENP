...
static bool make_packet(enp_packet_t *packet, enp_packet_type_t type,
						uint8_t ttl, enp_sequence_t sequence) {
	...
	if (type == ENP_PACKET_ACK) {
		...
		enp_ack_payload_init(ack, sequence, 0x00000001U);
...
static bool captured_packet(enp_packet_t *packet) {
	if ((packet == NULL) || (s_tx_length == 0U) ||
		(s_tx_length > sizeof(packet->buffer))) {
		return false;
	}

	memset(packet, 0, sizeof(*packet));
	memcpy(enp_packet_data(packet), s_tx_buffer, s_tx_length);
...
void app_main(void) {
	...
	if (!install_route(&routes)) {
		...
		ESP_LOGI(TAG,
				 "PASS: active route destination C -> next hop B installed");
	}

	if (!enp_routing_data_path_init(&routing_path, &routes, &transport,
	...
	if (enp_data_plane_init(&data_plane, &context, &routing_path,
	...
	if (!make_packet(&data_packet, ENP_PACKET_APPLICATION, TEST_TTL,
					 TEST_SEQUENCE)) {
	...
	if (enp_data_plane_process(&data_plane, &data_packet, &source) != ESP_OK) {
		ESP_LOGE(TAG, "FAIL: first DATA forwarded");
		pass = false;
	} else if (s_tx_count != 1U || s_tx_destination != TEST_NEXT_HOP) {
		ESP_LOGE(TAG,
				 "FAIL: first DATA forwarding selected wrong next hop/count");
		pass = false;
	} else {
	...
	if (!captured_packet(&captured)) {
		...
		if ((captured_header == NULL) || (original_header == NULL) ||
			...
			ESP_LOGE(TAG, "FAIL: DATA forwarding did not preserve "
						  "identity/decrement TTL");
			pass = false;
		} else {
			ESP_LOGI(TAG, "PASS: DATA forwarding decremented TTL 8 -> 7 and "
						  "preserved identity");
		}
	}
	...
	if (!make_packet(&ack_packet, ENP_PACKET_ACK, TEST_TTL, TEST_SEQUENCE)) {
		ESP_LOGE(TAG, "FAIL: ACK packet constructed");
		pass = false;
	} else {
		ESP_LOGI(
			TAG,
			"PASS: ACK packet constructed with same source+sequence as DATA");
	}

	if (enp_data_plane_process(&data_plane, &ack_packet, &source) != ESP_OK) {
		...
		ESP_LOGI(TAG,
				 "PASS: ACK forwarded independently of DATA duplicate domain");
	}
	...
	enp_packet_init(&local_packet, ENP_PACKET_APPLICATION, &local_address);

	enp_header_t *local_header = enp_packet_header(&local_packet);
	...
	if (local_header != NULL) {
		...
		memcpy(enp_packet_payload(&local_packet), local_payload,
			   sizeof(local_payload));

		if (enp_packet_seal(&local_packet, sizeof(local_payload)) != ESP_OK) {
			ESP_LOGE(TAG, "FAIL: local packet construction");
			pass = false;
		} else if (enp_data_plane_process(&data_plane, &local_packet,
										  &source) != ESP_OK ||
	...
	if (enp_duplicate_count(&data_plane.data_duplicates,
	...
	if (enp_duplicate_count(&data_plane.ack_duplicates,
		...
		ESP_LOGI(
			TAG,
			"PASS: ACK duplicate domain contains one independent identity");
	}
