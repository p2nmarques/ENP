 ...
 *
  ...
  * B suppresses duplicate → B recovers cached ACK → A receives exactly one recovered ACK.
  *
  */
  ...
  #include <stdbool.h>
    ...
    */
    ...
    static esp_err_t build_data_packet(
           enp_packet_t *packet,
           enp_sequence_t packet_sequence,
   ...
   static esp_err_t send_data_from_a_sequence(
           enp_sequence_t packet_sequence,
           uint32_t application_sequence,
           bool record_local_identity)
   {
       ...
       enp_packet_t packet;
       esp_err_t err = build_data_packet(&packet, packet_sequence, application_sequence);
       if (err != ESP_OK) {
   ...
   static esp_err_t send_duplicate_test_packet_twice(
           enp_sequence_t packet_sequence,
           uint32_t application_sequence)
   {
       enp_packet_t packet;
       ...
       ESP_LOGI(TAG,
                "DUP TX #1: origin=A destination=C seq=%lu app_seq=%lu",
                (unsigned long)packet_sequence,
                (unsigned long)application_sequence);
       ...
       ESP_LOGI(TAG,
                "DUP TX #2 (identical packet): origin=A destination=C seq=%lu app_seq=%lu",
                (unsigned long)packet_sequence,
                (unsigned long)application_sequence);
   ...
   static void start_duplicate_test(void)
   {
       ...
       esp_err_t err = send_duplicate_test_packet_twice(
               E3_DUPLICATE_PACKET_SEQ_1,
       ...
       ESP_LOGI(TAG,
                "NEW DATA: origin=A destination=C seq=%lu app_seq=%u",
                (unsigned long)E3_DUPLICATE_PACKET_SEQ_2,
                (unsigned)E3_DUPLICATE_APP_SEQ_2);

       err = send_data_from_a_sequence(
               E3_DUPLICATE_PACKET_SEQ_2,
               E3_DUPLICATE_APP_SEQ_2,
               true);
       if (err != ESP_OK) {
           s_test_failed = true;
       ...
       if (s_ack_delivered_count != 2U) {
           ESP_LOGE(TAG,
                    "FAIL: expected exactly 2 ACK deliveries at A, got %lu",
                    (unsigned long)s_ack_delivered_count);
           s_test_failed = true;
       } else {
           ESP_LOGI(TAG, "PASS: A received exactly 2 ACKs for unique DATA packets");
       }
   ...
   static esp_err_t send_ack_packet(
           enp_packet_t *packet,
           enp_rreq_node_t next_hop)
   {
       if (packet == NULL) {
   ...
   static esp_err_t forward_cached_retry_ack(void)
   {
       ...
       if (err == ESP_OK) {
           ...
           ESP_LOGI(TAG,
                    "E3.3.6 ACK RECOVERY: B forwarded cached ACK source=C data_seq=%lu ack_seq=%lu ttl=%u",
   ...
   static esp_err_t send_ack_from_c(
           ...
           bool duplicate_for_test)
   {
       ...
       if (err == ESP_OK) {
           ...
           ESP_LOGI(TAG,
                    "ACK TX #1: origin=C destination=A seq=%lu data_seq=%lu app_seq=%lu ttl=%u",
                    (unsigned long)header->sequence,
           ...
           if (duplicate_for_test) {
               ...
               ESP_LOGI(TAG,
                        "ACK TX #2 (identical packet): origin=C destination=A seq=%lu data_seq=%lu app_seq=%lu",
                        (unsigned long)header->sequence,
                        (unsigned long)ack.data_packet_sequence,
                        (unsigned long)ack.application_sequence);
               err = send_ack_packet(
           ...
           const enp_transport_address_t *source)
   {
       ...
       if (CONFIG_ENP_E3_NODE_ID == E3_NODE_B &&
           ...
           ESP_LOGI(TAG,
                    "E3.3.6 FAULT INJECTION: B dropped first ACK data_seq=%lu ack_seq=%lu",
                    (unsigned long)ack.data_packet_sequence,
                    (unsigned long)header->sequence);
       ...
       if (ack_duplicate_check(packet)) {
           ESP_LOGW(TAG,
                    "ACK RX: duplicate suppressed source=%u seq=%lu",
       ...
       ESP_LOGI(TAG,
                "ACK RX: origin=%u destination=%u seq=%lu data_seq=%lu app_seq=%lu ttl=%u",
                (unsigned)header->source.node,
       ...
       if (header->destination.node == CONFIG_ENP_E3_NODE_ID) {
           ...
           if (s_retry_test_active) {
               if (ack.data_packet_sequence != s_retry_data_packet_sequence ||
                   ack.application_sequence != E3_RETRY_APP_SEQ) {
                   ESP_LOGE(TAG,
                            "FAIL: E3.3.6 recovered ACK mismatch data_seq=%lu app_seq=%lu",
                            (unsigned long)ack.data_packet_sequence,
                            (unsigned long)ack.application_sequence);
                   s_test_failed = true;
                   return;
           ...
           if (s_duplicate_test_active) {
               ...
               if (index >= s_duplicate_expected_count ||
                   ack.data_packet_sequence != s_duplicate_expected_sequences[index] ||
                   ack.application_sequence != s_duplicate_expected_app_sequences[index]) {
                   ESP_LOGE(TAG,
                            "FAIL: duplicate-test ACK mismatch index=%lu data_seq=%lu app_seq=%lu",
                            (unsigned long)index,
                            (unsigned long)ack.data_packet_sequence,
                            (unsigned long)ack.application_sequence);
                   s_test_failed = true;
               ...
               ++s_ack_delivered_count;
               ESP_LOGI(TAG,
                        "PASS: duplicate-test ACK %lu matches DATA sequence/app sequence",
                        (unsigned long)s_ack_delivered_count);
               return;
           }
       ...
       if (!lookup_route_next_hop(destination, &next_hop)) {
           ESP_LOGE(TAG,
                    "FAIL: ACK route unavailable at node %u for destination %u",
       ...
       if (err != ESP_OK) {
           ESP_LOGE(TAG, "FAIL: ACK forwarding reseal: %s",
                    esp_err_to_name(err));
           s_test_failed = true;
           return;
       }
       ...
       ESP_LOGI(TAG,
                "ACK FWD: node=%u destination=%u next-hop=%u ttl=%u",
                (unsigned)CONFIG_ENP_E3_NODE_ID,
                (unsigned)header->destination.node,
                (unsigned)next_hop.node_id,
   ...
   static void process_data(
            ...
            const enp_transport_address_t *source)
    {
        ...
        if (header->source.node == E3_NODE_A &&
            ...
            CONFIG_ENP_E3_NODE_ID != E3_NODE_A &&
            !s_retry_test_active) {
            ...
            ESP_LOGI(TAG, "E3.3.6 %c ROLE START: DATA retransmission / ACK recovery",
        ...
        if (header->destination.node == CONFIG_ENP_E3_NODE_ID) {
            ...
            if (s_retry_test_active && CONFIG_ENP_E3_NODE_ID == E3_NODE_C &&
                ...
                if (!expected) {
                    ESP_LOGE(TAG, "FAIL: C unexpected duplicate-test application sequence %lu",
                             (unsigned long)data_header.application_sequence);
                    s_test_failed = true;
                    return;
                }
            } else if (data_header.application_sequence != s_data_application_sequence) {
            ...
            if (s_duplicate_test_active) {
                ESP_LOGI(TAG,
                         "PASS: C unique DATA delivery count=%lu",
    ...
    static void start_data_test(void)
    {
        ...
        if (route == NULL || route->state != ENP_ROUTE_STATE_ACTIVE) {
            ESP_LOGE(TAG, "FAIL: E3.3.3 requires an active A -> C route; run e3 start first");
            return;
        }
   ...
   static void start_retry_test(void)
   {
       ...
       ESP_LOGI(TAG, "Test: B deliberately drops first ACK; A retransmits identical DATA");
       ESP_LOGI(TAG, "Expected: C delivers DATA once; B suppresses retry; cached ACK recovers delivery");
       ESP_LOGI(TAG, "======================================");
       ...
       if (s_retry_first_ack_dropped) {
           ...
           ESP_LOGW(TAG,
                    "E3.3.6: ACK-drop event not yet observed locally; proceeding with protocol timeout/retry");
       }
   ...
   static void neighbor_announce_task(void *argument)
    ...
    static void control_task(void *argument)
    {
        ...
        for (;;) {
            ESP_LOGI(TAG, "Type 'e3 start' for route discovery, 'e3 data', 'e3 dup', 'e3 ackdup' or 'e3 retry' on node A");
            ...
            if (strncmp(line, "e3 start", 8U) == 0) {
                ...
                ESP_LOGI(TAG, "Commands: e3 start | e3 data | e3 dup | e3 ackdup | e3 retry | e3 routes");
            }
        }
    }
