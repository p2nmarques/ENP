...
#include <string.h>
 ...
 static esp_err_t controlled_deinit(void)
 {
     return ESP_OK;
 }

 static esp_err_t controlled_send(
 ...
 static esp_err_t controlled_set_receive_callback(
 ...
 {
     .init = controlled_init,
     .deinit = controlled_deinit,
     .send = controlled_send,
     .set_receive_callback = controlled_set_receive_callback
 };

 static esp_err_t application_process(
 ...
 static esp_err_t discovery_process(
         enp_context_t *context,
         const enp_packet_t *packet,
         const enp_transport_address_t *source)
 {
 ...
 static const enp_service_t s_ack_service =
 {
     .name = "test_ack",
 ...
 void app_main(void)
 {
     ...
     ESP_LOGI(TAG, "Transport RX -> DATA/ACK data plane; other packets -> dispatcher");
    ...
    static enp_receive_path_t receive_path;
     ...
     expect(enp_dispatcher_init(&context) == ESP_OK,
     ...
     expect(enp_route_table_init(&routes),
            "route table initialized", &pass);

     enp_route_metric_t metric;
     expect(enp_route_metric_init(
                    &metric,
     ...
     expect(enp_routing_data_path_init(
                    &routing_path,
                    &routes,
                    &s_transport,
                    resolve_transport,
     ...
     s_rx_callback(&source, enp_packet_data_const(&packet), enp_packet_length(&packet));
     expect(s_application_calls == app_before + 1U,
            "local DATA reached dispatcher local service through data plane", &pass);
     ...
     expect(s_ack_calls == ack_before + 1U,
            "local ACK reached dispatcher local service through data plane", &pass);
     ...
     s_rx_callback(&source, enp_packet_data_const(&packet), enp_packet_length(&packet));
     expect(s_discovery_calls == discovery_before + 1U,
            "normal dispatcher duplicate cache suppressed duplicate discovery", &pass);
     ...
     expect(s_last_destination.length == 6U &&
            s_last_destination.value[0] == 2U,
            "forwarded DATA selected configured next-hop transport address", &pass);

     expect(enp_receive_path_deinit(&receive_path) == ESP_OK,
            "production receive path deinitialized", &pass);
     expect(enp_dispatcher_deinit() == ESP_OK,
            "dispatcher deinitialized", &pass);
     expect(enp_context_deinit(&context) == ESP_OK,
