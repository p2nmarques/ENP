...
static const char *TAG = "E3_UCAST";
...
#define E3_UCAST_PAYLOAD "ENP-E3.1-DIRECT-UNICAST"

#define E3_UCAST_RESPONSE_PAYLOAD "ENP-E3.1-UNICAST-RESPONSE"
 ...
 *     94:E6:86:0D:11:8C
 */
#define E3_GATEWAY_MAC {0x94U, 0xE6U, 0x86U, 0x0DU, 0x11U, 0x8CU}
 ...
 *     78:21:84:E6:19:84
 */
#define E3_SENSOR_MAC {0x78U, 0x21U, 0x84U, 0xE6U, 0x19U, 0x84U}
...
static bool wait_for_wifi(void) {
	...
	while (!enp_wifi_is_connected()) {
		...
		if ((now - start) >= E3_UCAST_TEST_TIMEOUT_MS) {
			return false;
		}
	}
...
static enp_transport_address_t make_mac_address(const uint8_t mac[6]) {
...
static void log_address(const char *label,
	...
	if (address->length != 6U) {
		ESP_LOGI(TAG, "%s: invalid length=%u", label,
	...
	ESP_LOGI(TAG,
			 ...
			 label, address->value[0], address->value[1], address->value[2],
...
static void e3_receive_callback(const enp_transport_address_t *source,
	...
	ESP_LOGI(TAG, "ENP RX callback: %u bytes", (unsigned)length);

	log_address("ENP RX source", source);
	 ...
	 */
	if (payload_equals(data, length, E3_UCAST_RESPONSE_PAYLOAD)) {
		s_response_received = true;

		ESP_LOGI(TAG, "Gateway received Sensor "
					  "unicast response");
	...
	ESP_LOGE(TAG, "Gateway received unexpected "
				  "payload");
	 ...
	 * unicast frame.
	 */
	if (payload_equals(data, length, E3_UCAST_PAYLOAD)) {
		s_unicast_received = true;

		ESP_LOGI(TAG, "Sensor received Gateway "
					  "direct unicast");
		 ...
		 *     ESP-NOW unicast
		 */
		const esp_err_t err = enp_transport_send(
			s_context.transport, source, E3_UCAST_RESPONSE_PAYLOAD,
			strlen(E3_UCAST_RESPONSE_PAYLOAD));
		...
		ESP_LOGI(TAG, "Sensor response submitted");
	...
	ESP_LOGE(TAG, "Sensor received unexpected "
				  "payload");
...
static bool initialize_platform(void) {
	...
	ESP_LOGI(TAG, "E3.1 DIRECT UNICAST DIAGNOSTIC");
...
static bool run_gateway_test(void) {
	const uint8_t sensor_mac[6] = E3_SENSOR_MAC;

	const enp_transport_address_t sensor = make_mac_address(sensor_mac);

	ESP_LOGI(TAG, "--------------------------------------");

	ESP_LOGI(TAG, "Gateway -> Sensor DIRECT UNICAST");

	log_address("Destination", &sensor);

	ESP_LOGI(TAG, "Payload: %s", E3_UCAST_PAYLOAD);
	...
	s_last_payload_length = 0U;

	/*
	 * Send directly to the Sensor MAC.
	 */
	const esp_err_t err =
		enp_transport_send(s_context.transport, &sensor, E3_UCAST_PAYLOAD,
						   strlen(E3_UCAST_PAYLOAD));

	CHECK(err == ESP_OK, "Gateway unicast send accepted");

	++s_tx_count;

	PASS("Gateway unicast frame submitted "
		 "to ESP-NOW");
	...
	CHECK(s_response_received, "Sensor unicast response received");

	CHECK(!s_payload_error, "Sensor response payload valid");

	CHECK(s_last_source.length == 6U, "Sensor response source has "
									  "ESP-NOW MAC length");
	...
	log_address("Sensor response source", &s_last_source);
...
static bool run_sensor_test(void) {
	const uint8_t gateway_mac[6] = E3_GATEWAY_MAC;

	const enp_transport_address_t gateway = make_mac_address(gateway_mac);

	ESP_LOGI(TAG, "--------------------------------------");

	ESP_LOGI(TAG, "Sensor waiting for Gateway "
				  "DIRECT UNICAST");

	log_address("Expected Gateway", &gateway);

	ESP_LOGI(TAG, "Expected payload: %s", E3_UCAST_PAYLOAD);
	...
	CHECK(!s_payload_error, "Gateway unicast payload valid");

	CHECK(s_last_source.length == 6U, "Gateway source has "
									  "ESP-NOW MAC length");

	CHECK(
		payload_equals(s_last_payload, s_last_payload_length, E3_UCAST_PAYLOAD),
		"Gateway unicast payload integrity");

	log_address("Actual Gateway source", &s_last_source);
	 ...
	 */
	PASS("Sensor response submitted "
		 "to Gateway");
...
static bool deinitialize_enp(void) {
	...
	CHECK(err == ESP_OK, "ENP context / ESP-NOW transport "
						 "deinitialized");
...
void app_main(void) {
	...
	if (ok) {
		ESP_LOGI(TAG, "ALL E3.1 GATEWAY "
					  "UNICAST DIAGNOSTIC TESTS PASSED");
	} else {
		ESP_LOGE(TAG, "E3.1 GATEWAY "
	...
	if (ok) {
		ESP_LOGI(TAG, "ALL E3.1 SENSOR "
					  "UNICAST DIAGNOSTIC TESTS PASSED");
	} else {
		ESP_LOGE(TAG, "E3.1 SENSOR "
					  "UNICAST DIAGNOSTIC TEST FAILED");
	}
