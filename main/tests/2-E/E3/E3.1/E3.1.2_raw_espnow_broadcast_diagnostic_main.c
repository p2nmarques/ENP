...
#define BROADCAST_PAYLOAD \
    "ENP-E3.1-RAW-BROADCAST"

#define RESPONSE_PAYLOAD \
    "ENP-E3.1-RAW-RESPONSE"
    ...
    int len)
{
    ...
    if (payload_equals(
		...
		ESP_LOGI(
		    ...
		    response_peer.peer_addr[0],
		    response_peer.peer_addr[1],
		    response_peer.peer_addr[2],
		    response_peer.peer_addr[3],
		    response_peer.peer_addr[4],
		    response_peer.peer_addr[5]);

        esp_err_t peer_err =
            esp_now_add_peer(&response_peer);

        if (peer_err == ESP_OK) {
            ESP_LOGI(
                TAG,
                "Gateway peer add: SUCCESS");
        }
        else if (peer_err == ESP_ERR_ESPNOW_EXIST) {
            ESP_LOGI(
                TAG,
                "Gateway peer add: ALREADY EXISTS");
        }
        else {
            s_payload_error = true;
        ...
        ESP_LOGI(
            TAG,
            "Sensor raw unicast response submitted");
    }
    else {
        s_payload_error = true;
        ...
        ESP_LOGE(
            TAG,
            "Sensor received unexpected payload");
    }
    ...
    if (payload_equals(
        ...
        ESP_LOGE(
            TAG,
            "Gateway received unexpected payload");
    }
...
static bool initialize_platform(void)
{
    ...
    ESP_LOGI(
        TAG,
        "SOURCE BUILD: PEER-FIX-V3");

#if CONFIG_DEVICE_ROLE_GATEWAY
    ESP_LOGI(
        TAG,
        "E3.1 RAW ESP-NOW BROADCAST");
    ESP_LOGI(
        TAG,
        "ROLE: GATEWAY");
#else
    ...
    if (esp_netif_init() != ESP_OK) {
        ESP_LOGE(
            TAG,
            "FAIL: ESP-NETIF initialization");
        return false;
    }

    ESP_LOGI(
        TAG,
        "PASS: ESP-NETIF initialized");

    if (esp_event_loop_create_default() != ESP_OK) {
        ESP_LOGE(
            TAG,
            "FAIL: default event loop initialization");
        return false;
    }

    ESP_LOGI(
        TAG,
        "PASS: default event loop initialized");

    if (enp_wifi_init() != ESP_OK) {
        ESP_LOGE(
            TAG,
            "FAIL: Wi-Fi initialization");
        return false;
    }

    ESP_LOGI(
        TAG,
        "PASS: Wi-Fi initialized");
...
static bool initialize_raw_espnow(void)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(
            TAG,
            "FAIL: esp_now_register_recv_cb(): %s",
            esp_err_to_name(err));
    ...
    ESP_LOGI(
        TAG,
        "PASS: raw RX callback registered");
...
static bool run_gateway_test(void)
{
    ...
    esp_err_t err =
        esp_now_send(
            s_broadcast_mac,
            (const uint8_t *)BROADCAST_PAYLOAD,
    ...
    if (!s_response_received) {
        ESP_LOGE(
            TAG,
            "FAIL: Sensor raw broadcast response not received");
    ...
    ESP_LOGI(
        TAG,
        "PASS: Sensor raw response received");

    if (s_payload_error) {
        ESP_LOGE(
            TAG,
            "FAIL: response payload error");
    ...
    if (!payload_equals(
        ...
        ESP_LOGE(
            TAG,
            "FAIL: response payload integrity");
    ...
    ESP_LOGI(
        TAG,
        "PASS: response payload integrity");
...
static bool run_sensor_test(void)
{
    ...
    ESP_LOGI(
        TAG,
        "Sensor waiting for RAW broadcast");
    ...
    if (!s_received) {
        ESP_LOGE(
            TAG,
            "FAIL: raw broadcast not received");
    ...
    ESP_LOGI(
        TAG,
        "PASS: raw broadcast received");

    if (s_payload_error) {
        ESP_LOGE(
            TAG,
            "FAIL: broadcast payload error");
    ...
    if (!payload_equals(
        ...
        ESP_LOGE(
            TAG,
            "FAIL: broadcast payload integrity");
    ...
    ESP_LOGI(
        TAG,
        "PASS: broadcast payload integrity");
...
static void deinitialize_raw_espnow(void)
{
    ...
    if (err == ESP_OK) {
        ESP_LOGI(
            TAG,
            "PASS: raw ESP-NOW deinitialized");
    }
    else {
        ESP_LOGE(
            TAG,
            "FAIL: raw ESP-NOW deinit: %s",
            esp_err_to_name(err));
    }
}
...
void app_main(void)
{
    ...
    if (ok) {
        ESP_LOGI(
            TAG,
            "ALL E3.1 RAW GATEWAY BROADCAST "
            "DIAGNOSTIC TESTS PASSED");
    }
    else {
        ESP_LOGE(
            TAG,
            "E3.1 RAW GATEWAY BROADCAST "
    ...
    if (ok) {
        ESP_LOGI(
            TAG,
            "ALL E3.1 RAW SENSOR BROADCAST "
            "DIAGNOSTIC TESTS PASSED");
    }
    else {
        ESP_LOGE(
            TAG,
            "E3.1 RAW SENSOR BROADCAST "
            "DIAGNOSTIC TEST FAILED");
    }
