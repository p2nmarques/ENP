 ...
 * terminology. Node 1 is the Gateway (A). Nodes 2 and 3 are configured
 * with DEVICE_ROLE_RELAY; node 2 is the Relay (B) and node 3 is the Sensor (C).
 *
 ...
 */
         ...
         enp_sequence_t packet_sequence)
 {
     ...
     if (err == ESP_OK) {
         ESP_LOGI(
                 TAG,
                 "RREQ TX: origin=A destination=C id=%lu hop=%u ttl=%u",
