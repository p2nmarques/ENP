 ...
 *
 * ENP v0.2 — E3.2.1 real three-node A -> B -> C discovery.
 *
 * Test roles use the existing DEVICE_ROLE_GATEWAY / DEVICE_ROLE_RELAY /
 * DEVICE_ROLE_SENSOR terminology. Node 1 is the Gateway (A). Nodes 2 and 3 are
 * configured with DEVICE_ROLE_RELAY; node 2 is the Relay (B) and node 3 is the
 * Sensor (C).
 *
 ...
 * discovery timeout.
 *
 * E3.2.1 exercises the existing R4-A.1, R4-B and R4-C processors over the
...
static esp_err_t send_rreq(const enp_routing_rreq_t *rreq,
	...
	enp_transport_address_t broadcast = {0};

	err = send_routing_packet(&packet, &broadcast);
	if (err == ESP_OK) {
		ESP_LOGI(TAG, "RREQ TX: origin=A destination=C id=%lu hop=%u ttl=%u",
...
static bool init_processors(void) {
	 ...
	 * forward a reply. The originator is always A for E3.2.1. */
...
static void process_rreq(const enp_packet_t *packet,
	...
	case ENP_RREQ_RESULT_FORWARD:
		...
		if (send_rreq(&forward_rreq, &header->source, header->sequence) ==
			ESP_OK) {
...
static bool start_discovery(void) {
	if (CONFIG_ENP_E3_NODE_ID != E3_NODE_A) {
		ESP_LOGW(TAG, "Only node A (node 1) can start E3.2.1");
		return false;
	}
	...
	ESP_LOGI(TAG, "======================================");
	ESP_LOGI(TAG, "E3.2.1 START: A -> B -> C discovery");
	...
	if (send_rreq(&rreq, &originator, s_active_rreq_packet_sequence) !=
		ESP_OK) {
		s_test_failed = true;
...
static void wait_for_discovery_result(void) {
	...
	while ((uint32_t)(enp_context_time_ms(&s_context) - started) <
		...
		if (enp_route_discovery_state(&s_discovery) ==
			...
			if (!s_test_failed) {
				...
				ESP_LOGI(TAG, "======================================");
				ESP_LOGI(TAG, "ALL E3.2.1 DISCOVERY TESTS PASSED");
				ESP_LOGI(TAG, "======================================");
			}
			return;
		}
		...
		if (enp_route_discovery_on_timeout(&s_discovery,
			...
			const enp_address_t originator = {.network = E3_NETWORK_ID,
											  .node = E3_NODE_A};
			(void)send_rreq(&retry, &originator, s_active_rreq_packet_sequence);
		}
	...
	s_test_failed = true;
	ESP_LOGE(TAG, "FAIL: E3.2.1 discovery exceeded 10 seconds");
	print_routes();
}
...
static void control_task(void *argument) {
	...
	ESP_LOGI(TAG, "READY");
	ESP_LOGI(TAG, "Waiting for E3.2.1 start command");
...
void app_main(void) {
	ESP_LOGI(TAG, "======================================");
	ESP_LOGI(TAG, "ENP v0.2 E3.2.1 REAL MULTI-HOP");
	ESP_LOGI(TAG, "======================================");
	...
	s_announce_task = xTaskCreateStatic(
		neighbor_announce_task, "e3_2_announce", E3_TASK_STACK_SIZE, NULL,
	...
	s_control_task = xTaskCreateStatic(
		control_task, "e3_2_control", E3_TASK_STACK_SIZE, NULL,
