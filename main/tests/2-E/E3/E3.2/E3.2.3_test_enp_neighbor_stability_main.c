/*
 * E3.2.3_test_enp_neighbor_stability_main_RELAY_ROLE.c
 *
 *  Created on: Aug 14, 2026
 *      Author: Pedro Marques
 *
 * ENP v0.2 — E3.2.3 real three-node neighbor discovery stability test.
 *
 * Roles:
 *   Node 1 = Gateway
 *   Node 2 = Relay
 ...
 *
 * E3.2.3 verifies that periodic discovery announcements refresh existing
 * neighbor entries instead of creating duplicate logical neighbors.
 * The periodic announcements remain enabled; repeated refreshes are silent.
 * Only the first observation of a logical neighbor is logged as
 * "Neighbor discovered".
 */
...
static const enp_neighbor_t *find_expected_neighbor(uint16_t node_id) {
	const enp_address_t address = {.network = E3_NETWORK_ID, .node = node_id};
...
static void begin_stability_test(void) {
	...
	ESP_LOGI(TAG, "--------------------------------------");
	ESP_LOGI(TAG, "E3.2.3 neighbor stability window started");
...
static void finish_stability_test(void) {
	...
	ESP_LOGI(TAG, "--------------------------------------");
	ESP_LOGI(TAG, "E3.2.3 neighbor stability results");
	...
	if (!create_ok) {
		ESP_LOGE(TAG,
				 "FAIL: expected exactly 2 neighbor entries to be created");
		s_test_failed = true;
	} else {
	...
	if (!table_ok) {
		ESP_LOGE(TAG,
				 "FAIL: neighbor table contains an invalid or duplicate state");
		s_test_failed = true;
	} else {
		ESP_LOGI(TAG,
				 "PASS: neighbor table remains stable with 2 active entries");
	}
	...
	ESP_LOGI(TAG, "======================================");
	if (s_test_failed) {
		ESP_LOGE(TAG, "E3.2.3 NEIGHBOR STABILITY TEST FAILED");
	} else {
		ESP_LOGI(TAG, "ALL E3.2.3 NEIGHBOR STABILITY TESTS PASSED");
	}
	ESP_LOGI(TAG, "======================================");
}
...
static void enp_receive_callback(const enp_transport_address_t *source,
	...
	const esp_err_t err = enp_neighbor_update(
		...
		(enp_role_t)discovery->role, discovery->capabilities, header->sequence,
		0, enp_context_time_ms(&s_context));
	...
	if (existing == NULL) {
		...
		ESP_LOGI(TAG, "Neighbor discovered: node=%lu transport-len=%u",
...
static void control_task(void *argument) {
	...
	ESP_LOGI(TAG, "READY");
	ESP_LOGI(TAG, "E3.2.3 neighbor stability test starts automatically");
...
void app_main(void) {
	ESP_LOGI(TAG, "======================================");
	ESP_LOGI(TAG, "ENP v0.2 E3.2.3 NEIGHBOR STABILITY");
	ESP_LOGI(TAG, "======================================");
