...
static bool validate_neighbor_table(bool require_refresh) {
	...
	for (size_t index = 0U; index < E3_EXPECTED_NEIGHBORS; ++index) {
		...
		if (memcmp(neighbor->transport_address.value,
				   s_baseline_transport[index], ESP_NOW_ETH_ALEN) != 0) {
			return false;
		}
...
static void begin_stability_test(void) {
	...
	for (size_t index = 0U; index < E3_EXPECTED_NEIGHBORS; ++index) {
		...
		memcpy(s_baseline_transport[index], neighbor->transport_address.value,
			   ESP_NOW_ETH_ALEN);
...
static void finish_stability_test(void) {
	...
	if (!table_ok) {
		ESP_LOGE(TAG,
				 "FAIL: neighbor identity or table state changed unexpectedly");
		s_test_failed = true;
	} else {
		ESP_LOGI(TAG,
				 "PASS: logical node -> transport identity remains stable");
		ESP_LOGI(TAG, "PASS: neighbor role identity remains stable");
		ESP_LOGI(TAG,
				 "PASS: neighbor last_seen_ms advanced for existing entries");
	}
...
static void control_task(void *argument) {
	...
	ESP_LOGI(TAG, "READY");
	ESP_LOGI(TAG,
			 "E3.2.4 neighbor identity consistency test starts automatically");
