 ...
 */
 ...
 static enp_route_destination_t destination(
     uint16_t network_id,
     uint16_t node_id)
 {
     ...
     uint32_t sequence)
 {
     ...
     return enp_route_table_insert(
         &ctx->table,
         &entry);
 }

 static bool mock_lookup_route(
     void *context,
     ...
     enp_rerr_route_info_t *route_info)
 {
     ...
     const enp_route_entry_t *entry =
         enp_route_table_lookup_const(
             &ctx->table,
             destination(
                 rerr_destination.network_id,
                 rerr_destination.node_id));
 ...
 static bool mock_invalidate_route(
     void *context,
     enp_rerr_destination_t rerr_destination)
 {
     node_context_t *ctx = context;
     ...
     node_context_t *ctx)
 {
     ...
     return enp_rerr_processor_init(
         processor,
         &callbacks);
 }
 ...
 static void test_newer_rerr_invalidates_route(void)
 {
     ...
     enp_routing_rerr_t rerr =
         make_rerr(
             3U,
             101U,
             ENP_ROUTE_ERROR_NEXT_HOP_UNREACHABLE);

     enp_rerr_result_t result =
         enp_rerr_processor_handle(
             &processor,
             &rerr);

     EXPECT_TRUE(
         result == ENP_RERR_RESULT_INVALIDATED,
         "newer RERR invalidates A route to C");

     EXPECT_TRUE(
         s_node_a.lookup_count == 1U,
         "newer RERR performs one route lookup");

     EXPECT_TRUE(
         s_node_a.invalidate_count == 1U,
     ...
     EXPECT_TRUE(
         route != NULL,
         "invalidated route entry is retained");

     if (route != NULL) {
         EXPECT_TRUE(
             route->state == ENP_ROUTE_STATE_STALE,
             "invalidated route becomes STALE");

         EXPECT_TRUE(
             route->next_hop.node_id == 2U,
             "invalidated route retains next hop B");
     }

     EXPECT_TRUE(
         enp_route_table_active_count(
             &s_node_a.table) == 0U,
 ...
 static void test_stale_rerr_does_not_invalidate(void)
 {
     ...
     EXPECT_TRUE(
         result == ENP_RERR_RESULT_IGNORED_STALE,
 ...
 static void test_unknown_route_rerr_is_ignored(void)
 {
     ...
     EXPECT_TRUE(
         result == ENP_RERR_RESULT_IGNORED_NO_ROUTE,
 ...
 void app_main(void)
 {
     ...
     ESP_LOGI(
         TAG,
         "ENP v0.2 E1.3 RERR integration");
     ...
     if (s_failures == 0) {
         ESP_LOGI(
             TAG,
             "ALL E1.3 RERR INTEGRATION TESTS PASSED");
     } else {
         ESP_LOGE(
             TAG,
             "%d E1.3 TEST(S) FAILED",
             s_failures);
     }
