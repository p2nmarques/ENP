 ...
 */
 ...
 static bool make_tx_packet(void)
 {
     ...
     const uint8_t payload[TEST_PAYLOAD_LENGTH] = {
         0xE3U, 0x37U, 0x0BU, 0x02U
     };

     memcpy(
             enp_packet_payload(&s_tx_packet),
             payload,
             sizeof(payload));
         ...
         size_t length)
 {
     ...
     if (!enp_address_equal(&header->source, &expected_source) ||
         ...
         memcmp(enp_packet_payload_const(&s_rx_packet),
                expected_payload,
                sizeof(expected_payload)) != 0)
     {
         ESP_LOGE(TAG,
                  "FAIL: received packet identity/content mismatch");
         s_rx_bad = true;
         return;
     }

     ESP_LOGI(TAG,
              "PASS: real ESP-NOW receiver validated DATA frame");
     ESP_LOGI(TAG,
              "PASS: source=network=%u node=%u destination=network=%u node=%u seq=0x%08" PRIX32,
 ...
 static bool run_sender(void)
 {
     ...
     {
         ESP_LOGE(TAG,
                  "FAIL: E2B peer MAC is still 00:00:00:00:00:00");
         ESP_LOGE(TAG,
                  "Set E2B_PEER_MAC_0..5 to Node 2 STA MAC and rebuild");
         return false;
     }
     ...
     ESP_LOGI(TAG, "PASS: real neighbor logical -> ESP-NOW MAC mapping installed");
 ...
 static bool finish_sender(void)
 {
     ...
     ESP_LOGI(TAG, "PASS: routing data path submitted DATA to real ESP-NOW transport");
