 ...
 */
     ...
     size_t length)
 {
     ...
     if (payload_equals(
         ...
         ESP_LOGI(
             TAG,
             "Sensor received Gateway broadcast");
         ...
         ESP_LOGI(
             TAG,
             "Sensor response sent to Gateway");
     }

     /*
      * The second frame is Gateway's unicast frame.
      */
     else if (payload_equals(
                  data,
                  length,
                  E3_UNICAST_PAYLOAD)) {
         ...
         ESP_LOGI(
             TAG,
             "Sensor received Gateway unicast");
     }
     ...
     if (payload_equals(
         ...
         ESP_LOGI(
             TAG,
             "Gateway received Sensor response");
     }
 ...
 static bool initialize_platform(void)
 {
     ...
     ESP_LOGI(
         TAG,
         "E3.1 Gateway ESP-NOW transport test");

 #else

     ESP_LOGI(
         TAG,
         "E3.1 Sensor ESP-NOW transport test");
 ...
 {
     const enp_transport_address_t broadcast = {
         .length = 0U
     };
     ...
     CHECK(
         !s_payload_error,
         "Gateway received valid response payload");
      ...
      */
     PASS(
         "Gateway unicast frame submitted to ESP-NOW");
 ...
 static bool run_sensor_test(void)
 {
     ...
     ESP_LOGI(
         TAG,
         "Sensor waiting for Gateway broadcast");
     ...
     CHECK(
         s_broadcast_received,
         "Gateway broadcast received");

     CHECK(
         !s_payload_error,
         "Gateway broadcast payload is valid");
     ...
     ESP_LOGI(
         TAG,
         "Sensor waiting for Gateway unicast");

     const uint32_t unicast_start =
         xTaskGetTickCount() * portTICK_PERIOD_MS;
     ...
     CHECK(
         !s_payload_error,
         "Gateway unicast payload is valid");
 ...
 static bool deinitialize_enp(void)
 {
     ...
     const esp_err_t err =
         enp_context_deinit(&s_context);

     CHECK(
         err == ESP_OK,
         "ENP context / ESP-NOW transport deinitialized");
 ...
 void app_main(void)
 {
     ...
     if (ok) {
         ESP_LOGI(
             TAG,
             "ALL E3.1 GATEWAY ESP-NOW TESTS PASSED");
     } else {
         ESP_LOGE(
             TAG,
             "E3.1 GATEWAY ESP-NOW TEST FAILED");
     }
     ...
     if (ok) {
         ESP_LOGI(
             TAG,
             "ALL E3.1 SENSOR ESP-NOW TESTS PASSED");
     } else {
         ESP_LOGE(
             TAG,
             "E3.1 SENSOR ESP-NOW TEST FAILED");
     }
