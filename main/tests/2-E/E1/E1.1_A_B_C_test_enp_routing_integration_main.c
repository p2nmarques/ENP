...
static enp_route_destination_t destination_from_rreq(enp_rreq_node_t n) {
	return (enp_route_destination_t){.network_id = n.network_id,
									 .node_id = n.node_id};
}
...
static bool install_route(enp_route_table_t *table,
	...
	enp_route_entry_t *existing = enp_route_table_lookup(table, destination);
...
static bool mock_is_duplicate(void *context, enp_rreq_node_t originator,
...
static bool mock_learn_reverse_route(void *context, enp_rreq_node_t originator,
	...
	return install_route(&ctx->table, destination_from_rreq(originator),
						 destination_from_rreq(next_hop), destination_sequence,
						 hop_count, lifetime_ms);
}
...
static bool mock_update_route(void *context, enp_rrep_node_t destination,
	...
	return install_route(
		&ctx->table,
		(enp_route_destination_t){.network_id = destination.network_id,
								  .node_id = destination.node_id},
		(enp_route_destination_t){.network_id = next_hop.network_id,
								  .node_id = next_hop.node_id},
		destination_sequence, (uint8_t)metric, lifetime_ms);
}

static bool mock_lookup_next_hop(void *context, enp_rrep_node_t destination,
								 enp_rrep_node_t *next_hop) {
	node_context_t *ctx = context;
...
static bool mock_discovery_complete(void *context, enp_rrep_node_t destination,
									enp_route_sequence_t destination_sequence) {
	node_context_t *ctx = context;
	(void)destination;
...
static enp_routing_rreq_t make_rreq(uint16_t destination_node,
									uint32_t request_id,
									uint32_t destination_sequence,
									uint8_t hop_count, uint8_t ttl) {
	return (enp_routing_rreq_t){.payload_version = ENP_ROUTING_PAYLOAD_VERSION,
...
static enp_routing_rrep_t make_rrep(uint16_t destination_node,
									uint32_t destination_sequence,
									uint8_t hop_count) {
	return (enp_routing_rrep_t){.payload_version = ENP_ROUTING_PAYLOAD_VERSION,
...
static const enp_route_entry_t *lookup_route(node_context_t *ctx,
											 uint16_t destination_node) {
	return enp_route_table_lookup_const(
		&ctx->table, (enp_route_destination_t){1U, destination_node});
}
...
static void test_basic_a_b_c_discovery(void) {
	...
	EXPECT_TRUE(enp_route_discovery_init(&discovery_a),
				"A discovery initialized");

	/*
	 * A requests a route to C.
	 */
	EXPECT_TRUE(enp_route_discovery_start(&discovery_a,
	...
	EXPECT_TRUE(enp_rreq_processor_init(&rreq_b, node(2U), &rreq_b_callbacks),
				"B RREQ processor initialized");

	enp_routing_rreq_t rreq_a = make_rreq(3U, 0xA001U, 100U, 0U, 8U);
	enp_routing_rreq_t rreq_b_out;

	EXPECT_TRUE(enp_rreq_processor_handle(&rreq_b, node(1U), node(1U), &rreq_a,
										  &rreq_b_out) ==
					ENP_RREQ_RESULT_FORWARD,
				"B forwards A's RREQ");

	EXPECT_TRUE(rreq_b_out.hop_count == 1U, "B increments RREQ hop count");

	EXPECT_TRUE(rreq_b_out.ttl == 7U, "B decrements RREQ TTL");
	...
	EXPECT_TRUE(b_to_a != NULL, "B learned reverse route to A");

	EXPECT_TRUE(b_to_a != NULL && b_to_a->next_hop.node_id == 1U,
				"B reverse route to A uses A as next hop");

	EXPECT_TRUE(b->reverse_updates == 1U, "B learned one reverse route");
	...
	EXPECT_TRUE(enp_rreq_processor_init(&rreq_c, node(3U), &rreq_c_callbacks),
				"C RREQ processor initialized");

	enp_routing_rreq_t rreq_c_out;

	EXPECT_TRUE(enp_rreq_processor_handle(&rreq_c, node(1U), node(2U),
										  &rreq_b_out,
	...
	EXPECT_TRUE(c_to_a != NULL, "C learned reverse route to A");

	EXPECT_TRUE(c_to_a != NULL && c_to_a->next_hop.node_id == 2U,
				"C reverse route to A uses B as next hop");

	EXPECT_TRUE(c->reverse_updates == 1U, "C learned one reverse route");
	 ...
	 */
	enp_routing_rrep_t rrep_c = make_rrep(3U, 100U, 0U);
	...
	EXPECT_TRUE(enp_rrep_processor_init(&rrep_b, rrep_node(2U), rrep_node(1U),
										&rrep_b_callbacks),
				"B RREP processor initialized");

	enp_routing_rrep_t rrep_b_out;

	EXPECT_TRUE(enp_rrep_processor_handle(&rrep_b, rrep_node(3U), &rrep_c,
										  &rrep_b_out) ==
					ENP_RREP_RESULT_FORWARD,
				"B forwards RREP toward A");

	EXPECT_TRUE(rrep_b_out.hop_count == 1U, "B increments RREP hop count");
	...
	EXPECT_TRUE(b_to_c != NULL, "B learned forward route to C");

	EXPECT_TRUE(b_to_c != NULL && b_to_c->next_hop.node_id == 3U,
				"B forward route to C uses C as next hop");

	EXPECT_TRUE(b->next_hop_lookups == 1U, "B performs one lookup toward A");
	...
	EXPECT_TRUE(enp_rrep_processor_init(&rrep_a, rrep_node(1U), rrep_node(1U),
										&rrep_a_callbacks),
				"A RREP processor initialized");

	enp_routing_rrep_t rrep_a_out;

	EXPECT_TRUE(enp_rrep_processor_handle(&rrep_a, rrep_node(2U), &rrep_b_out,
										  &rrep_a_out) ==
					ENP_RREP_RESULT_COMPLETE,
				"A receives RREP and completes discovery");

	EXPECT_TRUE(a->discovery_completions == 1U,
				"A discovery completion callback invoked once");

	EXPECT_TRUE(enp_route_discovery_on_rrep(
					&discovery_a, (enp_discovery_destination_t){1U, 3U}, 100U),
				"A discovery state accepts completed RREP");

	EXPECT_TRUE(enp_route_discovery_state(&discovery_a) ==
	...
	EXPECT_TRUE(a_to_c != NULL, "A learned forward route to C");

	EXPECT_TRUE(a_to_c != NULL && a_to_c->next_hop.node_id == 2U,
				"A forward route to C uses B as next hop");

	/*
	 * Final topology proof.
	 */
	EXPECT_TRUE(enp_route_table_active_count(&a->table) == 1U,
				"A has one active route");

	EXPECT_TRUE(enp_route_table_active_count(&b->table) == 2U,
				"B has reverse and forward routes");

	EXPECT_TRUE(enp_route_table_active_count(&c->table) == 1U,
...
void app_main(void) {
	...
	if (s_failures == 0) {
		...
		ESP_LOGE(TAG, "%d E1.1 A-B-C DISCOVERY TEST(S) FAILED", s_failures);
	}
