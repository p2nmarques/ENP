...
static bool install_reverse_route(node_context_t *ctx,
...
static bool mock_learn_reverse_route(void *context, enp_rreq_node_t originator,
	...
	return install_reverse_route(ctx, originator, next_hop, hop_count,
								 destination_sequence, lifetime_ms);
}
...
static bool init_processor(enp_rreq_processor_t *processor, node_context_t *ctx,
	...
	return enp_rreq_processor_init(processor, node(local_node_id), &callbacks);
}
...
static void test_duplicate_rreq(void) {
	...
	EXPECT_TRUE(init_processor(&processor, &s_node_b, 2U),
				"B RREQ processor initialized");

	input = make_rreq(3U,	   /* destination C */
					  0xD001U, /* request ID */
	...
	enp_rreq_result_t first_result =
		enp_rreq_processor_handle(&processor, node(1U), /* originator A */
								  node(1U),				/* immediate sender A */
								  &input, &output);

	EXPECT_TRUE(first_result == ENP_RREQ_RESULT_FORWARD,
				"first RREQ is accepted and forwarded");

	EXPECT_TRUE(s_node_b.duplicate_checks == 1U,
				"first RREQ performs one duplicate check");

	EXPECT_TRUE(s_node_b.reverse_updates == 1U,
				"first RREQ learns reverse route");

	EXPECT_TRUE(output.hop_count == 1U,
				"first forwarded RREQ increments hop count");

	EXPECT_TRUE(output.ttl == 7U, "first forwarded RREQ decrements TTL");

	const enp_route_entry_t *reverse_route = enp_route_table_lookup_const(
		&s_node_b.table, route_destination(node(1U)));

	EXPECT_TRUE(reverse_route != NULL,
				"B has reverse route to A after first RREQ");

	if (reverse_route != NULL) {
		EXPECT_TRUE(reverse_route->next_hop.node_id == 1U,
	...
	enp_rreq_result_t duplicate_result = enp_rreq_processor_handle(
		&processor, node(1U), node(1U), &input, &output);

	EXPECT_TRUE(duplicate_result == ENP_RREQ_RESULT_DROP_DUPLICATE,
				"duplicate RREQ is dropped");

	EXPECT_TRUE(s_node_b.duplicate_checks == 2U,
				"duplicate RREQ performs duplicate check");

	EXPECT_TRUE(s_node_b.reverse_updates == 1U,
				"duplicate RREQ does not learn reverse route again");

	EXPECT_TRUE(enp_route_table_active_count(&s_node_b.table) == 1U,
...
static void test_ttl_expiration_boundary(void) {
	...
	EXPECT_TRUE(init_processor(&processor, &s_node_c, 3U),
	 ...
	 */
	input =
		make_rreq(99U, /* not C, therefore forwarding would normally occur */
				  0xD002U, 100U, 4U, 1U);

	s_node_c.duplicate = false;

	enp_rreq_result_t result =
		enp_rreq_processor_handle(&processor, node(1U), /* A */
								  node(2U),				/* B */
								  &input, &output);

	EXPECT_TRUE(result == ENP_RREQ_RESULT_DROP_TTL,
				"TTL=1 RREQ is dropped at forwarding boundary");

	EXPECT_TRUE(s_node_c.reverse_updates == 1U,
				"TTL=1 RREQ still learns reverse route");

	EXPECT_TRUE(s_node_c.duplicate_checks == 1U,
				"TTL=1 RREQ performs duplicate check");

	const enp_route_entry_t *reverse_route = enp_route_table_lookup_const(
		&s_node_c.table, route_destination(node(1U)));

	EXPECT_TRUE(reverse_route != NULL,
				"C has reverse route to A after TTL drop");

	if (reverse_route != NULL) {
		EXPECT_TRUE(reverse_route->next_hop.node_id == 2U,
					"C reverse route to A uses B as next hop");

		EXPECT_TRUE(reverse_route->metric.valid &&
	 ...
	 */
	EXPECT_TRUE(result != ENP_RREQ_RESULT_FORWARD,
...
void app_main(void) {
	...
	if (s_failures == 0) {
		ESP_LOGI(TAG, "ALL E1.2 DUPLICATE + TTL TESTS PASSED");
	} else {
		ESP_LOGE(TAG, "%d E1.2 TEST(S) FAILED", s_failures);
	}
