 ...
 */
     ...
     size_t length)
 {
     ...
     s_mock.receive_callback(
         source,
         data,
         length);
 }

 static enp_transport_address_t make_address(
     uint8_t seed)
 {
 ...
 static void test_transport_initialization(void)
 {
     ...
     EXPECT_TRUE(
         s_mock.init_calls == 1U,
         "transport init callback invoked once");

     EXPECT_TRUE(
         s_mock.initialized,
         "mock transport reports initialized");

     EXPECT_TRUE(
         s_mock.last_config.network_id == 1U &&
 ...
 static void test_receive_registration(void)
 {
     ...
     EXPECT_TRUE(
         s_mock.callback_set_calls == 1U,
         "receive callback registration invoked once");

     EXPECT_TRUE(
         s_mock.callback_registered,
 ...
 static void test_send(void)
 {
     ...
     EXPECT_TRUE(
         s_mock.send_calls == 1U,
         "transport send callback invoked once");

     EXPECT_TRUE(
         s_mock.last_destination.length == destination.length &&
         memcmp(
             s_mock.last_destination.value,
             destination.value,
             destination.length) == 0,
         "send receives destination unchanged");

     EXPECT_TRUE(
         s_mock.last_payload_length == sizeof(payload) &&
         memcmp(
             s_mock.last_payload,
             payload,
             sizeof(payload)) == 0,
 ...
 static void test_context_integration(void)
 {
     ...
     EXPECT_TRUE(
         context.transport == &s_mock_transport,
     ...
     EXPECT_TRUE(
         enp_context_deinit(&context) == ESP_OK,
         "ENP context deinitializes");

     EXPECT_TRUE(
         s_mock.deinit_calls == 1U,
         "context deinitialization invokes transport deinit");

     EXPECT_TRUE(
         s_mock.deinitialized,
         "mock transport reports deinitialized");
 }
 ...
 static void test_context_invalid(void)
 {
     ...
     EXPECT_TRUE(
         enp_context_deinit(NULL) == ESP_ERR_INVALID_ARG,
 ...
 static void test_transport_deinitialization(void)
 {
     ...
     EXPECT_TRUE(
         enp_transport_deinit(
             &s_mock_transport) == ESP_OK,
     ...
     EXPECT_TRUE(
         s_mock.deinitialized,
         "transport reports deinitialized");
 }
 ...
 void app_main(void)
 {
     ...
     if (s_failures == 0) {
         ESP_LOGI(
             TAG,
             "ALL E2.1 MOCK TRANSPORT TESTS PASSED");
     } else {
         ESP_LOGE(
             TAG,
             "%d E2.1 TEST(S) FAILED",
             s_failures);
     }
