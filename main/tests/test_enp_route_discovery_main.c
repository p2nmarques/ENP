...
static const enp_discovery_destination_t DEST = {
    .network_id = 1U,
    .node_id = 2U
};

static const enp_discovery_destination_t OTHER_DEST = {
    .network_id = 1U,
    .node_id = 3U
};
...
static void test_initialization(void)
{
    ...
    EXPECT_TRUE(
        enp_route_discovery_state(&discovery) ==
            ENP_DISCOVERY_STATE_IDLE,
        "initial state is IDLE");

    EXPECT_TRUE(
        !enp_route_discovery_is_active(&discovery),
        "initialized discovery is inactive");

    EXPECT_TRUE(
        !enp_route_discovery_init(NULL),
...
static void test_start(void)
{
    ...
    EXPECT_TRUE(
        discovery.route_request_id == 0x1001U,
        "request ID stored");

    EXPECT_TRUE(
        discovery.destination_sequence == 55U,
    ...
    EXPECT_TRUE(
        discovery.deadline_ms == 1000U + ENP_DISCOVERY_TIMEOUT_MS,
...
static void test_start_validation(void)
{
    ...
    EXPECT_TRUE(
        !enp_route_discovery_start(
            &discovery,
            (enp_discovery_destination_t){0U, 0U},
            1U,
            1U,
            8U,
            0U),
...
static void test_timeout_and_retry(void)
{
    ...
    EXPECT_TRUE(
        !enp_route_discovery_on_timeout(
            &discovery,
            1000U + ENP_DISCOVERY_TIMEOUT_MS - 1U),
        "timeout before deadline does not retry");

    EXPECT_TRUE(
        enp_route_discovery_on_timeout(
            &discovery,
            1000U + ENP_DISCOVERY_TIMEOUT_MS),
        "timeout at deadline starts retry");

    EXPECT_TRUE(
        discovery.retry_count == 1U,
    ...
    EXPECT_TRUE(
        enp_route_discovery_on_timeout(
            &discovery,
            1000U + 2U * ENP_DISCOVERY_TIMEOUT_MS),
        "second timeout starts retry");

    EXPECT_TRUE(
        discovery.retry_count == 2U,
        "second retry increments retry count");

    EXPECT_TRUE(
        enp_route_discovery_on_timeout(
            &discovery,
            1000U + 3U * ENP_DISCOVERY_TIMEOUT_MS),
        "third timeout starts retry");

    EXPECT_TRUE(
        discovery.retry_count == 3U,
...
static void test_failure(void)
{
    ...
    for (uint32_t i = 0U; i < ENP_MAX_RETRIES; ++i) {
        uint32_t now =
            ENP_DISCOVERY_TIMEOUT_MS * (i + 1U);

        EXPECT_TRUE(
            enp_route_discovery_on_timeout(&discovery, now),
            "timeout schedules retry");
    }

    EXPECT_TRUE(
        discovery.retry_count == ENP_MAX_RETRIES,
...
static void test_reuse(void)
{
    ...
    EXPECT_TRUE(
        discovery.route_request_id == 0x5002U,
        "reuse replaces request ID");

    EXPECT_TRUE(
        discovery.destination.node_id == OTHER_DEST.node_id,
        "reuse replaces destination");

    EXPECT_TRUE(
        discovery.retry_count == 0U,
        "reuse resets retry count");
}
...
static void test_clock_wrap(void)
{
    ...
    EXPECT_TRUE(
        !enp_route_discovery_on_timeout(
            &discovery,
            start + ENP_DISCOVERY_TIMEOUT_MS - 1U),
        "discovery remains active across clock wrap");

    EXPECT_TRUE(
        enp_route_discovery_on_timeout(
            &discovery,
            start + ENP_DISCOVERY_TIMEOUT_MS),
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "Timeout: %u ms",
             (unsigned)ENP_DISCOVERY_TIMEOUT_MS);
    ESP_LOGI(TAG, "Max retries: %u",
             (unsigned)ENP_MAX_RETRIES);
    ESP_LOGI(TAG, "Default TTL: %u",
             (unsigned)ENP_DISCOVERY_DEFAULT_TTL);
    ESP_LOGI(TAG, "======================================");
    ...
    if (s_failures == 0) {
        ...
        ESP_LOGE(
            TAG,
            "%d ROUTE DISCOVERY STATE TEST(S) FAILED",
            s_failures);
    }
