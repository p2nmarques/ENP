...
static enp_routing_rreq_t make_rreq(uint16_t destination_node,
									uint32_t request_id, uint8_t hop_count,
									uint8_t ttl) {
...
static void setup(enp_rreq_processor_t *processor, mock_context_t *mock) {
	enp_rreq_processor_callbacks_t callbacks = {
		.context = mock,
		.is_duplicate = mock_is_duplicate,
		.learn_reverse_route = mock_learn_reverse_route};

	enp_rreq_node_t local = {.network_id = 1U, .node_id = 3U};
...
static void test_initialization(void) {
	...
	mock_context_t mock = {.reverse_route_ok = true};
	...
	EXPECT_TRUE(enp_rreq_processor_init(&processor, (enp_rreq_node_t){1U, 3U},
										&callbacks),
				"processor initialization");

	EXPECT_TRUE(
		!enp_rreq_processor_init(NULL, (enp_rreq_node_t){1U, 3U}, &callbacks),
		"initialization rejects NULL");

	EXPECT_TRUE(!enp_rreq_processor_init(&processor, (enp_rreq_node_t){0U, 0U},
										 &callbacks),
				"invalid local node rejected");

	EXPECT_TRUE(
		!enp_rreq_processor_init(&processor, (enp_rreq_node_t){1U, 3U}, NULL),
...
static void test_forward(void) {
	...
	enp_rreq_result_t result =
		enp_rreq_processor_handle(&processor, (enp_rreq_node_t){1U, 1U},
								  (enp_rreq_node_t){1U, 2U}, &input, &output);

	EXPECT_TRUE(result == ENP_RREQ_RESULT_FORWARD, "new RREQ is forwarded");

	EXPECT_TRUE(output.hop_count == 3U, "forwarded RREQ increments hop count");

	EXPECT_TRUE(output.ttl == 4U, "forwarded RREQ decrements TTL");

	EXPECT_TRUE(mock.duplicate_checks == 1U, "duplicate check performed once");

	EXPECT_TRUE(mock.reverse_route_updates == 1U, "reverse route learned once");

	EXPECT_TRUE(mock.last_originator.node_id == 1U,
				"reverse route stores originator");

	EXPECT_TRUE(mock.last_next_hop.node_id == 2U,
				"reverse route stores immediate sender");

	EXPECT_TRUE(mock.last_hop_count == 3U,
				"reverse route stores incremented hop count");

	EXPECT_TRUE(mock.last_sequence == input.destination_sequence,
				"reverse route stores destination sequence");

	EXPECT_TRUE(mock.last_lifetime == input.route_lifetime_ms,
...
static void test_destination_reply(void) {
	...
	EXPECT_TRUE(enp_rreq_processor_handle(&processor, (enp_rreq_node_t){1U, 1U},
										  (enp_rreq_node_t){1U, 2U}, &input,
										  &output) == ENP_RREQ_RESULT_REPLY,
				"destination RREQ requests RREP");

	EXPECT_TRUE(mock.reverse_route_updates == 1U,
...
static void test_duplicate(void) {
	enp_rreq_processor_t processor;
	mock_context_t mock = {.duplicate = true, .reverse_route_ok = true};
	...
	EXPECT_TRUE(mock.reverse_route_updates == 0U,
...
static void test_invalid(void) {
	...
	EXPECT_TRUE(enp_rreq_processor_handle(&processor, (enp_rreq_node_t){0U, 0U},
										  (enp_rreq_node_t){1U, 2U}, &input,
										  &output) == ENP_RREQ_RESULT_REJECT,
...
static void test_route_update_failure(void) {
	enp_rreq_processor_t processor;
	mock_context_t mock = {.reverse_route_ok = false};
...
void app_main(void) {
	...
	if (s_failures == 0) {
		...
		ESP_LOGE(TAG, "%d RREQ PROCESSOR TEST(S) FAILED", s_failures);
	}
