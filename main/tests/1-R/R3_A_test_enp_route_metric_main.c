...
static void test_initialization(void) {
	...
	EXPECT_TRUE(enp_route_metric_init(&metric, ENP_ROUTE_METRIC_HOP_COUNT),
				"metric initialization");

	EXPECT_TRUE(metric.valid, "initialized metric is valid");
	EXPECT_TRUE(metric.type == ENP_ROUTE_METRIC_HOP_COUNT,
				"metric type is HOP_COUNT");
	EXPECT_TRUE(metric.value == 0U, "initial value is zero");

	EXPECT_TRUE(!enp_route_metric_init(NULL, ENP_ROUTE_METRIC_HOP_COUNT),
				"initialization rejects NULL");

	EXPECT_TRUE(
		!enp_route_metric_init(&metric, (enp_route_metric_type_t)0xFFFFU),
		"unsupported metric type rejected");

	EXPECT_TRUE(!metric.valid, "unsupported metric remains invalid");
}
...
static void test_hop_addition(void) {
	...
	EXPECT_TRUE(enp_route_metric_add_hop(&metric), "add first hop");

	EXPECT_TRUE(metric.value == 1U, "first hop produces metric 1");

	EXPECT_TRUE(enp_route_metric_add_hop(&metric), "add second hop");

	EXPECT_TRUE(metric.value == 2U, "second hop produces metric 2");

	for (uint32_t i = 2U; i < 100U; ++i) {
		EXPECT_TRUE(enp_route_metric_add_hop(&metric),
					"add hop within normal range");
	}

	EXPECT_TRUE(metric.value == 100U, "100-hop sequence produces metric 100");
}
...
static void test_comparison(void) {
	...
	EXPECT_TRUE(enp_route_metric_compare(&zero, &one) < 0,
				"zero-hop route is better than one-hop route");

	EXPECT_TRUE(enp_route_metric_compare(&one, &two) < 0,
				"one-hop route is better than two-hop route");

	EXPECT_TRUE(enp_route_metric_compare(&two, &one) > 0,
				"two-hop route is worse than one-hop route");

	EXPECT_TRUE(enp_route_metric_compare(&one, &equal) == 0,
...
static void test_invalid_and_null(void) {
	...
	enp_route_metric_t invalid = {
		.type = ENP_ROUTE_METRIC_HOP_COUNT, .value = 10U, .valid = false};

	enp_route_metric_init(&valid, ENP_ROUTE_METRIC_HOP_COUNT);

	EXPECT_TRUE(!enp_route_metric_add_hop(&invalid),
				"invalid metric rejects hop addition");

	EXPECT_TRUE(enp_route_metric_compare(&valid, &invalid) < 0,
				"valid metric is better than invalid metric");

	EXPECT_TRUE(enp_route_metric_compare(&invalid, &valid) > 0,
				"invalid metric is worse than valid metric");

	EXPECT_TRUE(enp_route_metric_compare(&invalid, &invalid) == 0,
				"two invalid metrics compare equal");

	EXPECT_TRUE(!enp_route_metric_add_hop(NULL), "hop addition rejects NULL");

	EXPECT_TRUE(enp_route_metric_compare(NULL, &valid) > 0,
				"NULL lhs is worse than valid metric");

	EXPECT_TRUE(enp_route_metric_compare(&valid, NULL) < 0,
				"valid metric is better than NULL rhs");

	EXPECT_TRUE(enp_route_metric_compare(NULL, NULL) == 0,
...
static void test_overflow(void) {
	...
	EXPECT_TRUE(metric.value == ENP_ROUTE_METRIC_MAX_VALUE,
...
static void test_incompatible_types(void) {
	...
	enp_route_metric_t incompatible = {
		.type = (enp_route_metric_type_t)2U, .value = 1U, .valid = true};

	enp_route_metric_init(&hop, ENP_ROUTE_METRIC_HOP_COUNT);

	EXPECT_TRUE(enp_route_metric_compare(&hop, &incompatible) == 0,
				"incompatible metric types are not ranked");

	EXPECT_TRUE(enp_route_metric_compare(&incompatible, &hop) == 0,
...
void app_main(void) {
	...
	ESP_LOGI(TAG, "Maximum value: %u", (unsigned)ENP_ROUTE_METRIC_MAX_VALUE);
	ESP_LOGI(TAG, "======================================");
	...
	if (s_failures == 0) {
		...
		ESP_LOGE(TAG, "%d ROUTE METRIC TEST(S) FAILED", s_failures);
	}
