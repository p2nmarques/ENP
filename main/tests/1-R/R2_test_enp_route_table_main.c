...
static enp_route_entry_t make_entry(
...
static void test_initialization(void)
{
    ...
    EXPECT_TRUE(
        enp_route_table_count(&table) == 0U,
        "initial table count is zero");

    EXPECT_TRUE(
        enp_route_table_active_count(&table) == 0U,
        "initial active count is zero");

    EXPECT_TRUE(
        enp_route_table_init(NULL) == false,
...
static void test_insert_lookup(void)
{
    ...
    EXPECT_TRUE(
        enp_route_table_count(&table) == 1U,
        "count after insert");

    EXPECT_TRUE(
        enp_route_table_active_count(&table) == 1U,
        "active count after insert");

    found = enp_route_table_lookup(
        &table,
        entry.destination);

    EXPECT_TRUE(
        found != NULL,
        "lookup finds active route");

    EXPECT_TRUE(
        found != NULL &&
        found->next_hop.node_id == 10U,
        "lookup returns correct next hop");

    EXPECT_TRUE(
        found != NULL &&
        found->metric.value == 1U,
        "lookup returns correct metric");

    EXPECT_TRUE(
        enp_route_table_lookup(
            &table,
            (enp_route_destination_t){1U, 99U}) == NULL,
        "lookup misses unknown destination");

    EXPECT_TRUE(
        enp_route_table_insert(&table, &entry) == false,
...
static void test_update(void)
{
    ...
    EXPECT_TRUE(
        enp_route_table_update(&table, &updated),
    ...
    EXPECT_TRUE(
        found != NULL &&
        found->next_hop.node_id == 12U,
        "update changes next hop");

    EXPECT_TRUE(
        found != NULL &&
        found->metric.value == 2U,
        "update changes metric");

    EXPECT_TRUE(
        found != NULL &&
        found->route_sequence == 11U,
        "update changes sequence");

    {
        enp_route_entry_t unknown = make_entry(
            99U, 199U, 3U, 12U, 3000U);
...
static void test_remove(void)
{
    ...
    EXPECT_TRUE(
        enp_route_table_remove(&table, a.destination),
        "remove route");
    ...
    EXPECT_TRUE(
        enp_route_table_lookup(&table, a.destination) == NULL,
        "removed route is absent");

    EXPECT_TRUE(
        enp_route_table_lookup(&table, b.destination) != NULL,
...
static void test_expiration(void)
{
    ...
    EXPECT_TRUE(
        enp_route_table_expire(&table, 999U) == 0U,
        "no route expires before deadline");

    EXPECT_TRUE(
        enp_route_table_expire(&table, 1000U) == 1U,
    ...
    EXPECT_TRUE(
        enp_route_table_expire(&table, 2000U) == 1U,
...
static void test_capacity(void)
{
    ...
    for (uint16_t i = 0U; i < ENP_MAX_ROUTES; ++i) {
        enp_route_entry_t entry = make_entry(
            (uint16_t)(100U + i),
            (uint16_t)(200U + i),
            1U,
            (uint32_t)i,
    ...
    EXPECT_TRUE(
        enp_route_table_count(&table) == ENP_MAX_ROUTES,
        "table reaches ENP_MAX_ROUTES");

    {
        enp_route_entry_t extra = make_entry(
            999U, 1999U, 1U, 999U, 100000U);

        EXPECT_TRUE(
            !enp_route_table_insert(&table, &extra),
...
static void test_invalid_entries(void)
{
    ...
    EXPECT_TRUE(
        !enp_route_table_insert(&table, NULL),
        "NULL entry rejected");
}
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "Maximum routes: %u",
             (unsigned)ENP_MAX_ROUTES);
    ESP_LOGI(TAG, "======================================");
