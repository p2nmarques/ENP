...
static enp_routing_rrep_t make_rrep(uint16_t destination_node,
									uint32_t sequence, uint8_t hop_count,
									uint32_t lifetime) {
...
static void setup(enp_rrep_processor_t *processor, mock_context_t *mock,
				  uint16_t local_node, uint16_t originator_node) {
	...
	enp_rrep_processor_init(processor, (enp_rrep_node_t){1U, local_node},
							(enp_rrep_node_t){1U, originator_node}, &callbacks);
}

static void test_initialization(void) {
	enp_rrep_processor_t processor;
	mock_context_t mock = {
		.update_ok = true, .lookup_ok = true, .completion_ok = true};
	...
	EXPECT_TRUE(enp_rrep_processor_init(&processor, (enp_rrep_node_t){1U, 3U},
										(enp_rrep_node_t){1U, 1U}, &callbacks),
				"processor initialization");

	EXPECT_TRUE(!enp_rrep_processor_init(NULL, (enp_rrep_node_t){1U, 3U},
										 (enp_rrep_node_t){1U, 1U}, &callbacks),
				"initialization rejects NULL");

	EXPECT_TRUE(!enp_rrep_processor_init(&processor, (enp_rrep_node_t){0U, 0U},
										 (enp_rrep_node_t){1U, 1U}, &callbacks),
				"invalid local node rejected");

	EXPECT_TRUE(!enp_rrep_processor_init(&processor, (enp_rrep_node_t){1U, 3U},
										 (enp_rrep_node_t){0U, 0U}, &callbacks),
				"invalid originator rejected");

	EXPECT_TRUE(!enp_rrep_processor_init(&processor, (enp_rrep_node_t){1U, 3U},
										 (enp_rrep_node_t){1U, 1U}, NULL),
...
static void test_forward(void) {
	...
	enp_routing_rrep_t input = make_rrep(9U, 100U, 2U, 5000U);
	enp_routing_rrep_t output;

	EXPECT_TRUE(enp_rrep_processor_handle(&processor, (enp_rrep_node_t){1U, 8U},
										  &input,
										  &output) == ENP_RREP_RESULT_FORWARD,
				"RREP is forwarded toward originator");

	EXPECT_TRUE(mock.update_calls == 1U, "forward route updated once");

	EXPECT_TRUE(mock.last_destination.node_id == 9U,
				"route update stores RREP destination");

	EXPECT_TRUE(mock.last_next_hop.node_id == 8U,
				"route update stores previous hop");

	EXPECT_TRUE(mock.last_sequence == 100U,
				"route update stores destination sequence");

	EXPECT_TRUE(mock.last_hop_count == 2U,
				"route update stores RREP hop count");

	EXPECT_TRUE(mock.last_lifetime == 5000U,
				"route update stores route lifetime");

	EXPECT_TRUE(mock.last_metric == 2U, "route update stores metric");

	EXPECT_TRUE(mock.lookup_calls == 1U,
				"originator next-hop lookup performed");

	EXPECT_TRUE(mock.lookup_destination.node_id == 1U,
				"lookup targets discovery originator");

	EXPECT_TRUE(output.hop_count == 3U, "forwarded RREP increments hop count");

	EXPECT_TRUE(output.destination_node_id == input.destination_node_id,
...
static void test_complete_at_originator(void) {
	...
	enp_routing_rrep_t input = make_rrep(9U, 200U, 4U, 10000U);
	enp_routing_rrep_t output;

	EXPECT_TRUE(enp_rrep_processor_handle(&processor, (enp_rrep_node_t){1U, 2U},
										  &input,
										  &output) == ENP_RREP_RESULT_COMPLETE,
				"RREP completes discovery at originator");

	EXPECT_TRUE(mock.update_calls == 1U, "originator updates forward route");

	EXPECT_TRUE(mock.completion_calls == 1U,
				"discovery completion callback invoked");

	EXPECT_TRUE(mock.lookup_calls == 0U,
				"originator does not perform next-hop lookup");

	EXPECT_TRUE(mock.last_destination.node_id == 9U,
				"completion reports route destination");

	EXPECT_TRUE(mock.last_sequence == 200U,
...
static void test_invalid(void) {
	...
	enp_routing_rrep_t input = make_rrep(9U, 1U, 2U, 5000U);
	enp_routing_rrep_t output;
	...
	EXPECT_TRUE(enp_rrep_processor_handle(&processor, (enp_rrep_node_t){0U, 0U},
										  &input,
										  &output) == ENP_RREP_RESULT_REJECT,
...
static void test_no_route(void) {
	enp_rrep_processor_t processor;
	mock_context_t mock = {
		.update_ok = true, .lookup_ok = false, .completion_ok = true};

	setup(&processor, &mock, 3U, 1U);

	enp_routing_rrep_t input = make_rrep(9U, 10U, 2U, 5000U);
	enp_routing_rrep_t output;
	...
	EXPECT_TRUE(mock.update_calls == 1U, "destination route is still learned");

	EXPECT_TRUE(mock.lookup_calls == 1U, "originator lookup attempted");
}

static void test_update_failure(void) {
	enp_rrep_processor_t processor;
	mock_context_t mock = {
		.update_ok = false, .lookup_ok = true, .completion_ok = true};

	setup(&processor, &mock, 3U, 1U);

	enp_routing_rrep_t input = make_rrep(9U, 20U, 2U, 5000U);
	enp_routing_rrep_t output;
...
static void test_completion_failure(void) {
	enp_rrep_processor_t processor;
	mock_context_t mock = {
		.update_ok = true, .lookup_ok = true, .completion_ok = false};

	setup(&processor, &mock, 1U, 1U);

	enp_routing_rrep_t input = make_rrep(9U, 30U, 3U, 5000U);
	enp_routing_rrep_t output;
...
static void test_sequence_wrap_helper_behavior(void) {
	 ...
	 */
	enp_routing_rrep_t input = make_rrep(9U, 0x00000002U, 3U, 5000U);
	enp_routing_rrep_t output;
	...
	EXPECT_TRUE(mock.last_sequence == 0x00000002U,
				"wrapped sequence preserved");
}

static void test_wire_round_trip(void) {
	enp_routing_rrep_t input = make_rrep(9U, 0x12345678U, 3U, 9000U);
...
void app_main(void) {
	...
	if (s_failures == 0) {
		...
		ESP_LOGE(TAG, "%d RREP PROCESSOR TEST(S) FAILED", s_failures);
	}
