...
/* IG-G.8i three-node physical validation test
 * extern void ig_g_8i_test_start(enp_context_t *context);
 */
 ...
 */
         ...
         size_t length)
 {
     ...
     /*
      * Inspect the packet before deciding between local delivery
      * and routed forwarding.
      *
      * Packet validation remains authoritative in the destination
      * processing path and in the routing data path itself.
      */
     ...
     /*
      * A non-local application packet belongs to the routing
      * forwarding path.
      *
      * Discovery and all other packet types remain on the existing
      * dispatcher path.
      */
         ...
         (header->destination.node != s_context.network.local.id))
     {
         ...
         if (routing_path == NULL) {
             ESP_LOGW(TAG,
                      "Routing path unavailable: destination=%lu",
                      (unsigned long)header->destination.node);
             return;
         }

         ESP_LOGI(TAG,
                  "Forwarding application packet: "
                  "source=%lu destination=%lu ttl=%u",
         ...
         if (forward_err != ESP_OK) {
             ...
             ESP_LOGI(TAG,
                      "Packet forwarded: destination=%lu",
                      (unsigned long)header->destination.node);
         }
     ...
     /*
      * Existing local delivery path.
      *
      * Discovery packets and locally addressed application packets
      * continue to be handled by the dispatcher.
      */
     const esp_err_t err =
...
void app_main(void) {
   ...
   /* IG-G.8i test driver:
 	...
 	*   Node 2 -> diagnostic observer
 	*   Node 3 -> inactive test harness
 	*
 	* ig_g_8i_test_start() internally selects the appropriate
 	* role-specific behavior.
 	*
	* ig_g_8i_test_start(&s_context);
	*
	*/
