 ...
 */
 ...
 static esp_err_t enp_receive_path_local_process(
         void *context,
         ...
         const enp_transport_address_t *source)
 {
     ...
     return enp_dispatcher_dispatch_local(
             packet,
             source);
 }
         ...
         size_t length)
 {
     ...
     {
         return enp_data_plane_process(
                 &path->data_plane,
                 &packet,
                 source);
     }

     return enp_dispatcher_dispatch(
             &packet,
             source);
 }
