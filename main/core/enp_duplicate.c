...
static bool enp_duplicate_lock(
...
static void enp_duplicate_unlock(
        enp_duplicate_cache_t *cache)
{
    if ((cache != NULL) &&
        (cache->mutex != NULL))
...
static bool enp_duplicate_expired(
        const enp_duplicate_entry_t *entry,
        uint32_t now_ms)
{
...
static void enp_duplicate_expire_entries(
        enp_duplicate_cache_t *cache,
        uint32_t now_ms)
{
    for (size_t index = 0U;
         index < ENP_DUPLICATE_CACHE_SIZE;
         ++index)
    {
        if (enp_duplicate_expired(
                    &cache->entries[index],
                    now_ms))
        {
...
static size_t enp_duplicate_find(
        const enp_duplicate_cache_t *cache,
...
static size_t enp_duplicate_find_free(
        const enp_duplicate_cache_t *cache)
{
    for (size_t index = 0U;
         index < ENP_DUPLICATE_CACHE_SIZE;
...
static size_t enp_duplicate_find_oldest(
        const enp_duplicate_cache_t *cache,
        uint32_t now_ms)
{
         ...
         ++index)
    {
        ...
        const uint32_t age =
                (uint32_t)(now_ms - entry->seen_at_ms);
