...
static inline void enp_ack_payload_init(enp_ack_payload_t *ack,
										uint32_t data_packet_sequence,
										uint32_t application_sequence) {
	if (ack == NULL) {
		return;
...
static inline bool enp_ack_payload_valid(const enp_ack_payload_t *ack) {
	return ack != NULL && ack->payload_version == ENP_ACK_PAYLOAD_VERSION &&
		   ack->subtype == ENP_ACK_SUBTYPE_DATA &&
		   ack->status == ENP_ACK_STATUS_OK && ack->reserved == 0U &&
		   ack->data_packet_sequence != 0U && ack->application_sequence != 0U;
}
