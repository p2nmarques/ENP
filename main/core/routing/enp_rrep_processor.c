...
static bool node_equal(enp_rrep_node_t a, enp_rrep_node_t b) {
	return a.network_id == b.network_id && a.node_id == b.node_id;
}
...
bool enp_rrep_processor_init(enp_rrep_processor_t *processor,
	...
	if (processor == NULL || !node_valid(local_node) ||
		!node_valid(discovery_originator) || callbacks == NULL ||
		callbacks->update_route == NULL || callbacks->lookup_next_hop == NULL ||
...
enp_rrep_result_t enp_rrep_processor_handle(enp_rrep_processor_t *processor,
	...
	if (processor == NULL || rrep == NULL || forward_rrep == NULL ||
		!node_valid(previous_hop)) {
		return ENP_RREP_RESULT_REJECT;
	}

	if (rrep->payload_version != ENP_ROUTING_PAYLOAD_VERSION ||
		rrep->subtype != ENP_ROUTING_SUBTYPE_RREP || rrep->reserved_0 != 0U ||
		rrep->reserved_1 != 0U) {
		return ENP_RREP_RESULT_REJECT;
	}

	if (rrep->destination_network_id == 0U || rrep->destination_node_id == 0U) {
		return ENP_RREP_RESULT_REJECT;
	}
	...
	enp_rrep_node_t destination = {.network_id = rrep->destination_network_id,
	...
	if (!processor->callbacks.update_route(
			processor->callbacks.context, destination, previous_hop,
			rrep->destination_sequence, route_hop_count,
			rrep->route_lifetime_ms, (uint32_t)rrep->hop_count)) {
		return ENP_RREP_RESULT_REJECT;
	}
	 ...
	 */
	if (node_equal(processor->local_node, processor->discovery_originator)) {
		if (!processor->callbacks.discovery_complete(
				processor->callbacks.context, destination,
	...
	if (!processor->callbacks.lookup_next_hop(processor->callbacks.context,
