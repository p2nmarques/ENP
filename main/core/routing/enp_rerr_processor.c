...
enp_rerr_result_t enp_rerr_processor_handle(enp_rerr_processor_t *processor,
	...
	ESP_LOGI("RERRPROC",
			 ...
			 (unsigned)rerr->payload_version, (unsigned)rerr->subtype,
			 (unsigned)rerr->reason, (unsigned)rerr->reserved_0,
			 (unsigned long)rerr->reserved_1);

	if (rerr->subtype != ENP_ROUTING_SUBTYPE_RERR ||
		rerr->payload_version != ENP_ROUTING_PAYLOAD_VERSION ||
		rerr->reserved_0 != 0U || rerr->reserved_1 != 0U ||
		!reason_valid(rerr->reason)) {
		return ENP_RERR_RESULT_REJECT;
	}

	enp_rerr_destination_t destination = {.network_id =
											  rerr->unreachable_network_id,
	...
	if (!processor->callbacks.lookup_route(processor->callbacks.context,
										   destination, &route_info)) {
		return ENP_RERR_RESULT_REJECT;
	}
	...
	if (!sequence_is_newer_or_equal(rerr->destination_sequence,
									route_info.destination_sequence)) {
		return ENP_RERR_RESULT_IGNORED_STALE;
	}

	if (!processor->callbacks.invalidate_route(processor->callbacks.context,
											   destination)) {
		return ENP_RERR_RESULT_REJECT;
	}
