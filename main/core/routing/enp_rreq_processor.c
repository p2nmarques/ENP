...
static bool rreq_valid(const enp_routing_rreq_t *rreq) {
	if (rreq == NULL)
		return false;
	if (rreq->payload_version != ENP_ROUTING_PAYLOAD_VERSION)
		return false;
	if (rreq->subtype != ENP_ROUTING_SUBTYPE_RREQ)
		return false;
	if (rreq->destination_network_id == 0U || rreq->destination_node_id == 0U)
		return false;
	if (rreq->route_request_id == 0U)
		return false;
	if (rreq->ttl == 0U)
		return false;
	if (rreq->hop_count == UINT8_MAX)
		return false;
	return true;
}
...
bool enp_rreq_processor_init(enp_rreq_processor_t *processor,
	...
	if (processor == NULL || !node_valid(local_node) || callbacks == NULL ||
		callbacks->is_duplicate == NULL ||
		callbacks->learn_reverse_route == NULL) {
		return false;
	}
...
enp_rreq_result_t enp_rreq_processor_handle(enp_rreq_processor_t *processor,
	...
	if (processor == NULL || rreq == NULL || forward_rreq == NULL ||
		!node_valid(processor->local_node) || !node_valid(originator) ||
		!node_valid(immediate_sender) || !rreq_valid(rreq)) {
		return ENP_RREQ_RESULT_REJECT;
	}

	if (processor->callbacks.is_duplicate(processor->callbacks.context,
										  originator, rreq->route_request_id)) {
	...
	if (!processor->callbacks.learn_reverse_route(
			processor->callbacks.context, originator, immediate_sender,
			new_hop_count, rreq->destination_sequence,
