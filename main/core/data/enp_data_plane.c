...
static bool enp_data_plane_is_local(const enp_data_plane_t *plane,
									const enp_address_t *destination) {
	if ((plane == NULL) || (plane->context == NULL) || (destination == NULL)) {
		return false;
	}
...
static enp_duplicate_cache_t *
enp_data_plane_duplicate_cache(enp_data_plane_t *plane,
...
static void enp_data_plane_expire_ack_cache(enp_data_plane_t *plane,
											uint32_t now_ms) {
	for (size_t index = 0U; index < ENP_DATA_PLANE_ACK_CACHE_SIZE; ++index) {
		...
		if (entry->valid && (uint32_t)(now_ms - entry->stored_at_ms) >=
...
static size_t enp_data_plane_find_cached_ack(enp_data_plane_t *plane,
	...
	for (size_t index = 0U; index < ENP_DATA_PLANE_ACK_CACHE_SIZE; ++index) {
		...
		if (entry->valid && entry->data_sequence == data_sequence &&
...
static size_t enp_data_plane_find_ack_slot(enp_data_plane_t *plane,
										   uint32_t now_ms) {
...
static esp_err_t enp_data_plane_cache_ack(enp_data_plane_t *plane,
	...
	if ((plane == NULL) || (packet == NULL) || (header == NULL) ||
		(ack == NULL) || !enp_ack_payload_valid(ack)) {
		return ESP_ERR_INVALID_ARG;
	}

	size_t index = enp_data_plane_find_cached_ack(
		plane, &header->destination, ack->data_packet_sequence, now_ms);
...
static const enp_packet_t *enp_data_plane_cached_ack_for_data(
	enp_data_plane_t *plane, const enp_packet_t *data_packet, uint32_t now_ms) {
	const enp_header_t *header = enp_packet_header_const(data_packet);

	if ((header == NULL) || ((header->flags & ENP_FLAG_ACK_REQUIRED) == 0U)) {
		return NULL;
	}

	const size_t index = enp_data_plane_find_cached_ack(
		plane, &header->source, header->sequence, now_ms);
...
esp_err_t enp_data_plane_init(enp_data_plane_t *plane, enp_context_t *context,
	...
	if ((plane == NULL) || (context == NULL) || (routing_path == NULL)) {
		return ESP_ERR_INVALID_ARG;
	}

	memset(plane, 0, sizeof(*plane));

	esp_err_t err = enp_duplicate_cache_init(&plane->data_duplicates);
	if (err != ESP_OK) {
		return err;
	}

	err = enp_duplicate_cache_init(&plane->ack_duplicates);
	if (err != ESP_OK) {
...
esp_err_t enp_data_plane_deinit(enp_data_plane_t *plane) {
	...
	esp_err_t data_err = enp_duplicate_cache_clear(&plane->data_duplicates);
	esp_err_t ack_err = enp_duplicate_cache_clear(&plane->ack_duplicates);
...
esp_err_t enp_data_plane_process(enp_data_plane_t *plane,
	...
	if ((plane == NULL) || (plane->context == NULL) ||
		(plane->routing_path == NULL) || (packet == NULL) || (source == NULL)) {
		return ESP_ERR_INVALID_ARG;
	}
	...
	enp_duplicate_cache_t *duplicate_cache =
		enp_data_plane_duplicate_cache(plane, (enp_packet_type_t)header->type);

	if (duplicate_cache != NULL) {
		bool duplicate = false;

		const esp_err_t duplicate_err = enp_duplicate_check_and_record(
			duplicate_cache, &header->source, header->sequence,
			enp_context_time_ms(plane->context), &duplicate);
			...
			if (header->type == (uint8_t)ENP_PACKET_APPLICATION) {
				const enp_packet_t *cached_ack =
					enp_data_plane_cached_ack_for_data(
						plane, packet, enp_context_time_ms(plane->context));

				if (cached_ack != NULL) {
					return enp_routing_data_path_forward(plane->routing_path,
														 cached_ack);
				}
			}
	...
	if (enp_data_plane_is_local(plane, &header->destination)) {
		if (plane->local_process == NULL) {
			return ESP_ERR_NOT_FOUND;
		}

		return plane->local_process(plane->context, packet, source);
	}

	if (header->type == (uint8_t)ENP_PACKET_ACK) {
		const esp_err_t cache_err = enp_data_plane_cache_ack(
			plane, packet, enp_context_time_ms(plane->context));
	...
	return enp_routing_data_path_forward(plane->routing_path, packet);
}
