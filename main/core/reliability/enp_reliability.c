 ...
 */
 ...
 static enp_reliability_transaction_t *find_transaction(
         enp_reliability_handle_t handle)
 {
     ...
     for (size_t i = 0U;
          i < ENP_RELIABILITY_MAX_TRANSACTIONS;
          ++i)
     {
         if (s_transactions[i].active &&
             s_transactions[i].handle == handle)
         {
 ...
 static bool packet_is_reliable_data(
         const enp_packet_t *packet)
 {
     ...
     if (header->sequence == 0U ||
         header->source.node == ENP_NODE_BROADCAST ||
 ...
 static bool ack_matches_transaction(
         const enp_reliability_transaction_t *transaction,
         const enp_packet_t *ack_packet)
 {
     ...
     const enp_header_t *header =
             enp_packet_header_const(ack_packet);
     ...
     if (!enp_address_equal(&header->source,
                            &transaction->destination) ||
         !enp_address_equal(&header->destination,
                            &transaction->origin))
     {
         return false;
     }
 ...
 static void complete_transaction(
         enp_reliability_transaction_t *transaction,
         enp_reliability_state_t state,
         enp_reliability_result_t result)
 {
     if (transaction == NULL)
 ...
 bool enp_reliability_set_result_callback(
         enp_reliability_result_fn result,
         void *user_context)
 {
     if (!s_initialized)
     {
         ...
         enp_reliability_handle_t *handle)
 {
     ...
     transaction->deadline_ms =
             now_ms + ENP_RELIABILITY_ACK_TIMEOUT_MS;

     /* The initial submission is not a retry. */
     const esp_err_t err =
             s_submit(&transaction->packet, s_submit_context);
 ...
 bool enp_reliability_get_retry_count(
         enp_reliability_handle_t handle,
         uint8_t *retry_count)
 {
 ...
 static esp_err_t selftest_submit(
         const enp_packet_t *packet,
         void *user_context)
 {
     (void)user_context;
 ...
 static bool selftest_make_data(
         enp_packet_t *packet)
 {
     const enp_address_t origin = {
         .network = SELFTEST_NETWORK_ID,
 ...
 bool enp_reliability_self_test(void)
 {
     ...
     if (!selftest_make_ack(&ack) ||
         !enp_reliability_process_ack(&ack, 2050U) ||
