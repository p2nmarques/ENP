...
static const char *TAG = "enp_maintenance";

static enp_context_t *s_context = NULL;
static TaskHandle_t s_task = NULL;
static StaticTask_t s_task_control;
static StackType_t s_task_stack[ENP_MAINTENANCE_TASK_STACK_SIZE];
static volatile bool s_running = false;

static void enp_maintenance_task(void *argument) {
	enp_context_t *context = (enp_context_t *)argument;
	...
	while (s_running) {
		...
		const uint32_t notification =
			ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(ENP_DISCOVERY_INTERVAL_MS));
		...
		const esp_err_t err = enp_service_discovery_send(context);

		if (err != ESP_OK) {
			ESP_LOGW(TAG, "Periodic discovery failed: %s",
					 esp_err_to_name(err));
		} else {
			ESP_LOGD(TAG, "Periodic discovery sent");
		}
	}
...
esp_err_t enp_maintenance_init(enp_context_t *context) {
	...
	s_task = xTaskCreateStatic(
		enp_maintenance_task, "enp_maint", ENP_MAINTENANCE_TASK_STACK_SIZE,
		context, ENP_MAINTENANCE_TASK_PRIORITY, s_task_stack, &s_task_control);
